/*
    ...
    @file data_board_to_idx_multi_phase_experiment_edax_canonical.cpp
        Board-data converter for the isolated Edax-linear experiment with
        Edax-style reflection sharing
    @date 2026
...
inline void edax_calc_canonical_features(Board *board, uint16_t res[]) {
    ...
    for (int feature = 0; feature < ADJ_N_FEATURES; ++feature) {
        int raw_idx = adj_pick_pattern(b_arr, feature);
...
int main(int argc, char *argv[]) {
    std::cerr << EVAL_DEFINITION_NAME << "_canonical" << std::endl;
    std::cerr << EVAL_DEFINITION_DESCRIPTION << " with Edax reflection sharing" << std::endl;
    if (argc < 10) {

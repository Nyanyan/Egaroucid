...
inline uint64_t calc_some_mobility_hv(uint64_t ph, uint64_t oh){
    ...
    __m128i p1, o, res1, res2;
    p1 = _mm_set_epi64x(ph, pv) & _mm_set1_epi64x(0x7F7F7F7F7F7F7F7FULL);
    p1 = _mm_slli_epi64(p1, 1);
    o = _mm_set_epi64x(oh, ov);
    res1 = ~(p1 | o) & (_mm_add_epi64(p1, o & _mm_set1_epi64x(0x7F7F7F7F7F7F7F7FULL)));
    
    horizontal_mirror_double(&ph, &pv);
    p1 = _mm_set_epi64x(ph, pv) & _mm_set1_epi64x(0x7F7F7F7F7F7F7F7FULL);
    p1 = _mm_slli_epi64(p1, 1);
    horizontal_mirror_m128(&o);
    res2 = ~(p1 | o) & (_mm_add_epi64(p1, o & _mm_set1_epi64x(0x7F7F7F7F7F7F7F7FULL)));

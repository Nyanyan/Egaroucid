/*
    ...
    @file current_fm_tri_type_score_speed_check.cpp
        Isolated speed check for current-model tri-type FM scoring
    @date 2026
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::cerr << "usage: current_fm_tri_type_score_speed_check <cross.egev4@same.egev4@count.egev4@cross_weight@same_weight@count_weight> [iterations] [case_count]" << std::endl;
        return 1;
    }
    ...
    run_benchmark("tri_type_fm", cases, iterations, current_fm_score_from_ids);
    return 0;
}

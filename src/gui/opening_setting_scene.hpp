...
class Opening_setting : public App::Scene {
    private:
        ...
        std::vector<ImageButton> move_up_buttons;  // 上へ移動ボタン
        std::vector<ImageButton> move_down_buttons;  // 下へ移動ボタン
        ...
        TextAreaEditState folder_name_area;
        ...
        // Result structure for drag and drop operations
        struct OpeningDragResult {
            ...
            bool drop_on_parent = false;
        };
    
    public:
        Opening_setting(const InitData& init) : IScene{ init } {
            add_button.init(GO_BACK_BUTTON_BACK_SX - 110, GO_BACK_BUTTON_SY, GO_BACK_BUTTON_WIDTH, GO_BACK_BUTTON_HEIGHT, GO_BACK_BUTTON_RADIUS, language.get("opening_setting", "add"), 25, getData().fonts.font, getData().colors.white, getData().colors.black);
            add_folder_button.init(GO_BACK_BUTTON_BACK_SX, GO_BACK_BUTTON_SY, GO_BACK_BUTTON_WIDTH, GO_BACK_BUTTON_HEIGHT, GO_BACK_BUTTON_RADIUS, language.get("in_out", "new_folder"), 20, getData().fonts.font, getData().colors.white, getData().colors.black);
        ...
        void update() override {
            ...
            if (creating_folder) {
                ...
                std::string folder_name_str = folder_name_area.text.narrow();
                ...
                SimpleGUI::TextArea(folder_name_area, Vec2{OPENING_SETTING_SX + OPENING_SETTING_LEFT_MARGIN + 150, sy + OPENING_SETTING_HEIGHT / 2 - 17}, SizeF{400, 30}, SimpleGUI::PreferredTextAreaMaxChars);
            ...
            if (!adding_elem && !editing_elem && !creating_folder) {
                auto res = handle_drag_and_drop();
                if (res.drop_completed) {
                    if (res.drop_on_parent) {
                        if (res.is_dragging_opening) {
                            move_opening_to_parent(res.dragged_opening_index);
                        } else if (res.is_dragging_folder) {
                            move_folder_to_parent(res.dragged_folder_name.narrow());
                        }
                    } else if (!res.drop_target_folder.empty()) {
                        if (res.is_dragging_opening) {
                            move_opening_to_folder(res.dragged_opening_index, res.drop_target_folder.narrow());
                        } else if (res.is_dragging_folder) {
                            move_folder_to_folder_target(res.dragged_folder_name.narrow(), res.drop_target_folder.narrow());
                        }
                    }
                }
            }

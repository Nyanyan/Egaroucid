/*
    ...
    @file current_fm_tri_type_count_by_type_stream12_score_compare.cpp
        Direct consistency check for count-by-pattern-type tri-type FM scalar scoring
        and streaming dim12 SIMD scoring.
    @date 2026
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::cerr << "usage: current_fm_tri_type_count_by_type_stream12_score_compare <cross.egev4@same.egev4@count.egct@cross_weight@same_weight@count_weight> [cases] [seed]" << std::endl;
        return 1;
    }
#if !USE_SIMD
    ...
    if (current_fm_cross_file.dim != 12 || current_fm_same_file.dim != 12 || current_fm_count_type_file.dim != 12) {
        std::cerr << "stream12 checker expects all banks to be dim12, found dims="
                  << current_fm_cross_file.dim << ','
                  << current_fm_same_file.dim << ','
                  << current_fm_count_type_file.dim << std::endl;
        return 1;
    }

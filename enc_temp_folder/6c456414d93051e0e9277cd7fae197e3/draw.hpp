...
struct ExplorerDragState {
    ...
    void reset_drag_preparation() {
        if (dragged_game_index >= 0 || !dragged_folder_name.empty()) {
            std::cerr << "Resetting drag preparation (was game: " << dragged_game_index 
                      << ", folder: " << (dragged_folder_name.empty() ? "none" : dragged_folder_name.narrow()) << ")" << std::endl;
        }
...
inline ExplorerDrawResult HandleDragDrop(
    ...
    if (drag_state.is_dragging && !MouseL.pressed()) {
        ...
        if (dropped_on_parent) {
            ...
            res.isDraggingGame = drag_state.is_dragging_game;
            res.isDraggingFolder = drag_state.is_dragging_folder;
            res.draggedGameIndex = drag_state.dragged_game_index;
            res.draggedFolderName = drag_state.dragged_folder_name;
...
inline void UpdateMouseStateAndDragStart(ExplorerDragState& drag_state, ExplorerDrawResult& res) {
    ...
    if (drag_state.mouse_just_released && !drag_state.is_dragging) {
        std::cerr << "Mouse released, resetting drag preparation" << std::endl;
    ...
    if (MouseL.pressed() && !drag_state.is_dragging) {
        if ((drag_state.dragged_game_index >= 0 || !drag_state.dragged_folder_name.empty()) && 
            ...
            std::cerr << "Checking drag distance: " << distance << " (threshold: " << ExplorerDragState::DRAG_THRESHOLD << ")" << std::endl;
            if (distance > ExplorerDragState::DRAG_THRESHOLD) {
                ...
                if (drag_state.dragged_game_index >= 0) {
                    ...
                    res.draggedGameIndex = drag_state.dragged_game_index;
                    std::cerr << "Game drag started: " << drag_state.dragged_game_index << ", distance: " << distance << std::endl;
                    ...
                    res.draggedFolderName = drag_state.dragged_folder_name;
                    std::cerr << "Folder drag started: " << drag_state.dragged_folder_name.narrow() << ", distance: " << distance << std::endl;
                }
            }
        } else {
            // Debug output when conditions are not met (but only occasionally to avoid spam)
            static int debug_counter = 0;
            if (debug_counter++ % 60 == 0) {  // Print every 60 frames
                if (drag_state.dragged_game_index < 0 && drag_state.dragged_folder_name.empty()) {
                    std::cerr << "No drag target prepared" << std::endl;
                }
                if (drag_state.drag_start_pos.x == 0 && drag_state.drag_start_pos.y == 0) {
                    std::cerr << "No drag start position" << std::endl;
                }
            }
        }
    }
    
    // DO NOT reset drag preparation here while mouse is still pressed!
    // Only reset when mouse is actually released (handled above)
}
...
inline ExplorerDrawResult DrawExplorerList(
    ...
    UpdateMouseStateAndDragStart(drag_state, res);
    
    // Handle drag drop detection
    ExplorerDrawResult drag_result = HandleDragDrop<FontsT, ColorsT, ResourcesT, LanguageT>(
        drag_state, folders_display, scroll_manager, itemHeight, n_games_on_window, has_parent);
    if (drag_result.dropCompleted) {
        return drag_result;
    }
    ...
    // Draw list items
    ExplorerDrawResult list_result = DrawExplorerListItems<FontsT, ColorsT, ResourcesT, LanguageT>(
        folders_display, games, delete_buttons, scroll_manager, 
        itemHeight, n_games_on_window, has_parent, fonts, colors, resources, language,
    ...
    // Draw dragged items
    DrawDraggedItems<FontsT, ColorsT>(drag_state, games, itemHeight, fonts, colors);
...
template <class FontsT, class ColorsT, class ResourcesT, class LanguageT>
inline ExplorerDrawResult DrawExplorerListItems(
    const std::vector<String>& folders_display,
    const std::vector<Game_abstract>& games,
    std::vector<ImageButton>& delete_buttons,
    Scroll_manager& scroll_manager,
    ...
    for (int row = strt_idx_int; row < std::min(total_rows, strt_idx_int + n_games_on_window); ++row) {
        ...
        Rect rect(IMPORT_GAME_SX, sy, IMPORT_GAME_WIDTH, itemHeight);
        if (row % 2) {
        ...
        if (has_parent && (row - strt_idx_int) == 0 && row == 0) {
            ExplorerDrawResult parent_result = DrawParentFolder<FontsT, ColorsT>(rect, drag_state, fonts, colors, sy, itemHeight, click_state, current_time);
        ...
        // Handle folders
            ...
            if (folder_idx >= 0 && folder_idx < (int)folders_display.size()) {
                ExplorerDrawResult folder_result = DrawFolderItem<FontsT, ColorsT, ResourcesT>(
                    folders_display[folder_idx], rect, row, drag_state, fonts, colors, resources,
                    sy, itemHeight, click_state, current_time
                );
        ...
        // Handle games
            ...
            if (i >= 0 && i < (int)games.size()) {
                ExplorerDrawResult game_result = DrawGameItem<FontsT, ColorsT>(
...
inline ExplorerDrawResult DrawParentFolder(
    const Rect& rect,
    ExplorerDragState& drag_state,
    FontsT& fonts,
    ColorsT& colors,
...
inline ExplorerDrawResult DrawFolderItem(
    const String& fname,
    const Rect& rect,
    int row,
    ExplorerDragState& drag_state,
    ...
    if (drag_state.mouse_just_pressed && rect.contains(drag_state.current_mouse_pos) && 
        ...
        std::cerr << "Folder drag prepared: " << fname.narrow() 
                  << " at pos (" << drag_state.drag_start_pos.x << ", " << drag_state.drag_start_pos.y << ")" << std::endl;
    }
    ...
    if (rect.leftClicked() && !drag_state.is_dragging) {
        ...
        if (click_state.last_clicked_folder == fname && current_time - click_state.last_click_time < ExplorerClickState::DOUBLE_CLICK_TIME_MS) {
            ...
            std::cerr << "Folder double-click detected: " << fname.narrow() << std::endl;
            drag_state.reset_drag_preparation();
...
template <class FontsT, class ColorsT>
inline ExplorerDrawResult DrawGameItem(
    ...
    if (drag_state.mouse_just_pressed && rect.contains(drag_state.current_mouse_pos) && 
        ...
        drag_state.drag_start_pos = drag_state.current_mouse_pos;
        drag_state.drag_offset = drag_state.current_mouse_pos - Vec2(rect.x, rect.y);
        std::cerr << "Game drag prepared: " << game_index 
                  << " at pos (" << drag_state.drag_start_pos.x << ", " << drag_state.drag_start_pos.y << ")" << std::endl;
    }
    ...
    if (game_click_area.leftClicked() && !drag_state.is_dragging) {
        ...
        if (click_state.last_clicked_game_index == game_index && current_time - click_state.last_click_time < ExplorerClickState::DOUBLE_CLICK_TIME_MS) {
            // Double-click detected - clear drag preparation and execute double-click
            std::cerr << "Game double-click detected: " << game_index << std::endl;
            drag_state.reset_drag_preparation();

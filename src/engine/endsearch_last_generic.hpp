...
inline int last1(Search *search, uint64_t player, int alpha, uint_fast8_t p0) {
    ...
    const int y = p0 >> 3;
    int n_flip_both = 
        N_LAST_FLIP_BOTH[0][join_h_line(player, y)][x] + // both h
        N_LAST_FLIP_BOTH[0][join_v_line(player, x)][y] + // both v
        N_LAST_FLIP_BOTH[N_BIT_IDX_D7[p0]][join_d7_line(player, x + y)][std::min(y, 7 - x)] + // both d7
        N_LAST_FLIP_BOTH[N_BIT_IDX_D9[p0]][join_d9_line(player, x + 7 - y)][std::min(y, x)];  // both d9
    int n_flip = n_flip_both & 0xff;
    int score = 2 * (pop_count_ull(player) + n_flip + 1) - HW2;
    if (n_flip == 0) { // pass
        ++search->n_nodes;
#if USE_SEARCH_STATISTICS
        ++search->n_nodes_discs[63];

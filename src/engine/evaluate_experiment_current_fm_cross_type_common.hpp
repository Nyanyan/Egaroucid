/*
    ...
    @file evaluate_experiment_current_fm_cross_type_common.hpp
        Shared loader and scorer for the isolated current-model + cross-type FM experiment
    @date 2026
...
inline bool current_fm_load_egev4(const char *file, bool show_log) {
    if (show_log) {
        std::cerr << "current-model + cross-type FM experiment evaluation file " << file << std::endl;
    }
    FILE *fp;

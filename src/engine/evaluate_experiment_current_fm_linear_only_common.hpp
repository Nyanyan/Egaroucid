/*
    ...
    @file evaluate_experiment_current_fm_linear_only_common.hpp
        Shared loader and scorer for the isolated current-model EGEV4 linear-only experiment
    @date 2026
...
inline bool current_fm_load_egev4(const char *file, bool show_log) {
    if (show_log) {
        std::cerr << "current-model EGEV4 linear-only experiment evaluation file " << file << std::endl;
    }
    FILE *fp;
...
inline int current_fm_score_from_ids(const int phase_idx, const int active_ids[], const int n_active) {
    ...
    return std::clamp((int)std::llround(linear), -SCORE_MAX, SCORE_MAX);
}

...
このグラフから、オセロというゲームの終盤局面の質について読み取れることを[局面から棋力を推定できるか](./#ゲームの特性_局面から棋力を推定できるか)しました。
...
また、1手ごとにどれくらいの局面数があるかを考えます。$p$手目について考えうる合法な局面の数を試算すると、$2^4 \times {}_{60}\mathrm{C}_p \times 2^p$と考えることができます。第一項は最初に埋まっている4マスの組み合わせ、第二項は残りの60マスで$p$マスを石で埋める埋め方、第三項は$p$手目までで埋まった石の色の組み合わせです。これを$0\leq p \leq 60$で計算すると、以下のグラフのようになります。序盤から40手目付近までで局面の数が増えて、それ以降は終局に近づくにつれて局面数が少し減っています。青線は参考として書いた上界です。また、$p \leq 12$までは上で序盤の展開数を全て求めたので厳密な値がわかります。それもプロットしてみました。この上界はあまり精度が良くないとわかります。

<div class="centering_box">
    <img class="pic2" src="img/state_space_complexity_0.png">
</div>

さて、ここから初期盤面からの到達可能性を考慮して、上限の推定精度を上げましょう。オセロでは盤面上に存在する石は全て8近傍で連結であるという制約があります。そのため、盤面上に$n$個の石が存在するときの近傍の空きマス数$e(n)$を見積もります。まず、$n$個の石があるときの空きマス数が$64-n$個なので、$e(n) \leq 64-n$です。

そして、オセロで任意の連結している部分に石を追加して、8近傍で接しているマス数を最大化する局面を計算機によって求めます。このとき、序盤12手(石数は16個)はあり得る展開が全列挙できるので、序盤12手だけ展開を全列挙して、そこから石を連結していくようにしました。

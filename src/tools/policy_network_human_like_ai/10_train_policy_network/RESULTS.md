...
- Current completed total: 12,405 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 12,405 position
samples:
...
- 現在の完了数: 12,405 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭12,405局面サンプルの top-1 symmetric accuracy:

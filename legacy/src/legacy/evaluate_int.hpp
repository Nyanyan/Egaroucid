...
inline void predict_add(int canput, int sur0, int sur1, double dense0[n_add_dense0][n_add_input], double bias0[n_add_dense0], double dense1[n_add_dense1][n_add_dense0], double bias1[n_add_dense1], int res[n_add_dense1]){
    double hidden0[n_add_dense0], in_arr[n_add_input], res_double[n_add_dense1];
    int i, j;
    ...
    for (i = 0; i < n_add_dense1; ++i){
        ...
        res[i] = round(res_double[i] * float_weight);
    }
}
...
inline void init_evaluation2(){
    ...
    for (phase_idx = 0; phase_idx < n_phases; ++phase_idx){
        ...
        for (i = 0; i < n_all_input; ++i){
            getline(ifs, line);
            all_dense[phase_idx][i] = round(stof(line) * float_weight);
        }
        getline(ifs, line);
        all_bias[phase_idx] = round(stof(line) * float_weight * float_weight);
    }
    cerr << endl;
}
...
inline int mid_evaluate(const board *b){
    ...
    if (b->p)
        res = -res;
    return (res + round_plus) >> eval_shift;
}

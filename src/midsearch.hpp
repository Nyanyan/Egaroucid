...
int nega_alpha_ordering_nomemo(board *b, bool skipped, int depth, int alpha, int beta, bool use_mpc, double mpct_in, int *n_nodes){
    ...
    int g, v = -inf;
    for (board &nnb: nb){
        g = -nega_alpha_ordering_nomemo(&nnb, false, depth - 1, -beta, -alpha, use_mpc, mpct_in, n_nodes);
        if (beta <= g)
            return g;
        alpha = max(alpha, g);
        v = max(v, g);
...
int nega_alpha_ordering(board *b, bool skipped, const int depth, int alpha, int beta, bool use_multi_thread, bool use_mpc, double mpct_in, int *n_nodes){
    ...
    int first_alpha = alpha, g, v = -inf;
        ...
        if (use_multi_thread){
            ...
            for (int i = 0; i < canput; ++i){
                g = -nega_alpha_ordering(&nb[i], false, depth - 1, -beta, -alpha, false, use_mpc, mpct_in, n_nodes);
                alpha = max(alpha, g);
                if (beta <= alpha){
                    if (l < g)
    ...
    #else
        for (board &nnb: nb){
            g = -nega_alpha_ordering(&nnb, false, depth - 1, -beta, -alpha, false, use_mpc, mpct_in, n_nodes);
            alpha = max(alpha, g);
            if (beta <= alpha){
                if (l < alpha)
                    transpose_table.reg(b, hash, alpha, u);

...
class PolicyNetwork {
  public:
    ...
    std::array<float, 64> predict(uint64_t black, uint64_t white) const {
        if (layers_.empty()) {
            throw std::runtime_error("PolicyNetwork::predict called before load");
        }
        std::vector<float> current(128);
        for (int i = 0; i < 64; ++i) {
            current[i] = static_cast<float>((black >> (63 - i)) & 1ULL);
            current[64 + i] = static_cast<float>((white >> (63 - i)) & 1ULL);
        }

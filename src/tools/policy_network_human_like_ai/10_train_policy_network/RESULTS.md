...
- Current completed total: 17,725 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 17,725 position
samples:
...
- 現在の完了数: 17,725 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭17,725局面サンプルの top-1 symmetric accuracy:

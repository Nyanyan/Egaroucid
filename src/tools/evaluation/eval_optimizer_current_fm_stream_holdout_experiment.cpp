/*
    ...
    Current-linear fixed FM-only optimizer with streaming chunk shuffling.
...
void update_record(
    ...
    if (cfg.residual_clip > 0.0) {
        residual = clamp_value(residual, -cfg.residual_clip, cfg.residual_clip);
    }
...
void write_summary(
    ...
    out << "optimizer=current-linear-fixed FM-only streaming Adam\n";

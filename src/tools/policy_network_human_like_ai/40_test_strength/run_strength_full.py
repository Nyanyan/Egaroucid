#!/usr/bin/env python3
"""100対戦セット、ランダム打ちなしの総当たり戦を実行・再開する。"""
    ...
    with log_path.open("a", encoding="utf-8") as log:
        ...
        with subprocess.Popen(
            ...
            if proc.stdout is None:
                raise RuntimeError("failed to capture tournament output")
...
def make_command(args: argparse.Namespace) -> List[str]:
    ...
    if args.no_random_player:
        ...
        cmd.append("--include-random-player")
    if args.resume:
        cmd.append("--resume")
    if args.dry_run:
        cmd.append("--dry-run")
    ...
    if args.measure_move_stone_loss:
        cmd.append("--measure-move-stone-loss")
...
def make_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="100対戦セット、ランダム打ちなしの総当たり戦を実行・再開します。"
    )
    ...
    random_player_group.add_argument(
        ...
        action="store_true",
        help="ランダム打ちを除外します (既定値)。",
    )
    random_player_group.add_argument(
        "--include-random-player",
        dest="no_random_player",
        action="store_false",
        help="ランダム打ちを参加者に追加します。",
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=SCRIPT_DIR / "output" / "xot_100sets_16players",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="対戦中に1手当たり推定石損を測定します。既定では棋譜だけを保存します。",
    )

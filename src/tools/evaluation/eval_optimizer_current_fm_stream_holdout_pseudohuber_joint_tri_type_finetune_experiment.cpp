/*
    ...
    Current-linear fixed joint tri-type FM optimizer with streaming chunk
    shuffling, pseudo-Huber loss, and EGEV4 FM initialization experiment.
    ...
    - count-pattern bank: pairs between the stone-count feature and all board
      pattern features

    The tool writes three EGEV4 files, one for each vector bank, so the tri-type
    FM evaluator can load them as cross.egev4@same.egev4@count.egev4@w_cross@w_same@w_count.
    Unlike the base joint-tri-type optimizer, this variant initializes all
    vector banks from existing EGEV4 files and then fine-tunes them.
*/
...
constexpr int EGEV4_TIMESTAMP_SIZE = 14;
...
constexpr uint64_t FM_BANK_SALT_COUNT = 0x434f554e545f464dULL; // "COUNT_FM"
...
struct Config {
    ...
    std::string init_cross_egev4;
    std::string init_same_egev4;
    std::string init_count_egev4;
    std::string output_cross_egev4;
    std::string output_same_egev4;
    std::string output_count_egev4;
    std::string summary;
...
[[noreturn]] void usage_error(const std::string &message) {
    throw std::runtime_error(message + "\n"
        "usage: eval_optimizer_current_fm_stream_holdout_finetune_experiment "
        "--input-egev2 FILE --data-root DIR --phases 6-10 "
        "--init-cross-egev4 FILE --init-same-egev4 FILE --init-count-egev4 FILE "
        "--train-ids 0,1,2 --holdout-ids 3 "
        "--output-cross-egev4 FILE --output-same-egev4 FILE --output-count-egev4 FILE --summary FILE [options]");
}
...
Config parse_args(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--init-cross-egev4") {
            cfg.init_cross_egev4 = require_value(i, argc, argv);
        } else if (arg == "--init-same-egev4") {
            cfg.init_same_egev4 = require_value(i, argc, argv);
        } else if (arg == "--init-count-egev4") {
            cfg.init_count_egev4 = require_value(i, argc, argv);
        } else if (arg == "--phase") {
        ...
        } else if (arg == "--output-count-egev4") {
            cfg.output_count_egev4 = require_value(i, argc, argv);
        } else if (arg == "--summary") {
    ...
    if (cfg.init_cross_egev4.empty()) {
        usage_error("--init-cross-egev4 is required");
    }
    if (cfg.init_same_egev4.empty()) {
        usage_error("--init-same-egev4 is required");
    }
    if (cfg.init_count_egev4.empty()) {
        usage_error("--init-count-egev4 is required");
    }
    ...
    if (cfg.output_count_egev4.empty()) {
        usage_error("--output-count-egev4 is required");
    }
...
template<typename T>
T read_binary_at(std::ifstream &in, const size_t offset, const std::string &what) {
    T value{};
...
void read_exact_at(std::ifstream &in, const size_t offset, void *dst, const size_t bytes, const std::string &what) {
    in.seekg(static_cast<std::streamoff>(offset));
...
void load_initial_egev4_phase_vectors(
    const fs::path &path,
    const Config &cfg,
    const int phase,
    ...
    std::ifstream in(path, std::ios::binary);
    if (!in) {
        throw std::runtime_error("cannot open initial " + bank_name + " EGEV4: " + path.string());
    }
    char magic[4]{};
    read_exact_at(in, EGEV4_TIMESTAMP_SIZE, magic, sizeof(magic), "EGEV4 magic from " + path.string());
    if (std::memcmp(magic, "EGEV", 4) != 0) {
        throw std::runtime_error("unexpected EGEV4 magic in " + path.string());
    }

    const size_t header_offset = EGEV4_TIMESTAMP_SIZE + 4;
    const int32_t version = read_binary_at<int32_t>(in, header_offset, "EGEV4 version from " + path.string());
    const int32_t n_phases = read_binary_at<int32_t>(in, header_offset + 4, "EGEV4 phase count from " + path.string());
    const int32_t n_params = read_binary_at<int32_t>(in, header_offset + 8, "EGEV4 parameter count from " + path.string());
    const int32_t dim = read_binary_at<int32_t>(in, header_offset + 12, "EGEV4 dimension from " + path.string());
    if (version != VERSION_LINEAR_FM_INT16_INT8 || n_phases != N_PHASES || n_params != N_PARAMS_PER_PHASE || dim != cfg.dim) {
        throw std::runtime_error(
            "initial " + bank_name + " EGEV4 shape mismatch: version=" + std::to_string(version) +
            " phases=" + std::to_string(n_phases) +
            " params=" + std::to_string(n_params) +
            " dim=" + std::to_string(dim) +
            " expected version=" + std::to_string(VERSION_LINEAR_FM_INT16_INT8) +
            " phases=" + std::to_string(N_PHASES) +
            " params=" + std::to_string(N_PARAMS_PER_PHASE) +
            " dim=" + std::to_string(cfg.dim)
        );
    }

    const size_t linear_scales_offset = header_offset + 16;
    const size_t vector_scales_offset = linear_scales_offset + static_cast<size_t>(N_PHASES) * sizeof(float);
    const size_t payload_offset = vector_scales_offset + static_cast<size_t>(N_PHASES) * sizeof(float);
    const size_t record_stride = static_cast<size_t>(2 + cfg.dim);
    const size_t phase_stride = static_cast<size_t>(N_PARAMS_PER_PHASE) * record_stride;
    const size_t expected_size = payload_offset + static_cast<size_t>(N_PHASES) * phase_stride;
    const size_t actual_size = static_cast<size_t>(fs::file_size(path));
    if (actual_size != expected_size) {
        throw std::runtime_error("initial " + bank_name + " EGEV4 size mismatch: got " +
            std::to_string(actual_size) + " expected " + std::to_string(expected_size));
    }

    const float vector_scale = read_binary_at<float>(
        in,
        vector_scales_offset + static_cast<size_t>(phase) * sizeof(float),
        "EGEV4 vector scale from " + path.string()
    );
    std::vector<char> payload(phase_stride);
    read_exact_at(
        in,
        payload_offset + static_cast<size_t>(phase) * phase_stride,
        payload.data(),
        payload.size(),
        "EGEV4 phase payload from " + path.string()
    );

    for (int param_id = 0; param_id < N_PARAMS_PER_PHASE; ++param_id) {
        const size_t input_base = static_cast<size_t>(param_id) * record_stride + 2;
        const size_t output_base = static_cast<size_t>(param_id) * cfg.dim;
        for (int d = 0; d < cfg.dim; ++d) {
            const int8_t q = static_cast<int8_t>(payload[input_base + static_cast<size_t>(d)]);
            vectors[output_base + static_cast<size_t>(d)] = static_cast<float>(static_cast<double>(q) * vector_scale);
        }
...
struct CountInteractionScratch {
    std::vector<double> pattern_sums;
    std::vector<double> count_vec;

    explicit CountInteractionScratch(int dim)
        : pattern_sums(static_cast<size_t>(dim), 0.0),
          count_vec(static_cast<size_t>(dim), 0.0) {}

    void clear() {
        std::fill(pattern_sums.begin(), pattern_sums.end(), 0.0);
...
double predict_record(
    ...
    const std::vector<float> &count_vectors,
    const Record &record,
    int dim,
    double cross_weight,
    double same_weight,
    ...
    CountInteractionScratch &count_scratch
) {
    cross_scratch.clear();
    same_scratch.clear();
    count_scratch.clear();
    ...
    for (int feature_idx = 0; feature_idx < N_FEATURES; ++feature_idx) {
        ...
        if (pattern_type < 0) {
            for (int d = 0; d < dim; ++d) {
                count_scratch.count_vec[d] = count_vectors[base + d];
            }
            continue;
        }
        ...
        for (int d = 0; d < dim; ++d) {
            ...
            count_scratch.pattern_sums[d] += count_vectors[base + d];
        }
    }
    ...
    for (int d = 0; d < dim; ++d) {
        ...
        cross_interaction +=
            ...
            cross_same_type_interaction;
        count_interaction += count_scratch.pattern_sums[d] * count_scratch.count_vec[d];
    }
    return value
...
Metrics compute_metrics(
    ...
    CountInteractionScratch count_scratch(cfg.dim);
    for (int file_id : file_ids) {
        const fs::path path = data_path(root, phase, file_id);
        std::ifstream in(path, std::ios::binary);
        if (!in) {
        ...
        while (max_records_per_file <= 0 || n_file < static_cast<uint64_t>(max_records_per_file)) {
            ...
            const double pred = predict_record(
                linear, cross_vectors, same_vectors, count_vectors, record, cfg.dim,
...
void update_record(
    ...
    std::vector<float> &count_vectors,
    std::vector<float> &count_m,
    std::vector<float> &count_v,
    std::vector<uint8_t> &count_initialized,
    uint64_t &step,
    double &beta1_power,
    double &beta2_power,
    InteractionScratch &cross_scratch,
    ...
    for (int feature_idx = 0; feature_idx < N_PATTERN_FEATURES; ++feature_idx) {
        ...
        initialize_param(param_id, phase, cfg, FM_BANK_SALT_COUNT, count_vectors, count_m, count_v, count_initialized);
    }
    const uint32_t stone_count_param_id = record.active_ids[N_PATTERN_FEATURES];
    initialize_param(stone_count_param_id, phase, cfg, FM_BANK_SALT_COUNT, count_vectors, count_m, count_v, count_initialized);
    ...
    for (int feature_idx = 0; feature_idx < N_PATTERN_FEATURES; ++feature_idx) {
        ...
        for (int d = 0; d < cfg.dim; ++d) {
            ...
            const double count_vec = count_vectors[base + d];
            const double count_grad =
                -residual * cfg.count_weight * count_scratch.count_vec[d] +
                cfg.l2 * count_vec;
            const double next_count_m = 0.9 * count_m[base + d] + 0.1 * count_grad;
            const double next_count_v = 0.999 * count_v[base + d] + 0.001 * count_grad * count_grad;
            double next_count_vec = count_vec - lr_t * next_count_m / (std::sqrt(next_count_v) + 1.0e-8);
            next_count_vec *= decay;
            count_m[base + d] = static_cast<float>(next_count_m);
            count_v[base + d] = static_cast<float>(next_count_v);
            count_vectors[base + d] = static_cast<float>(next_count_vec);
        }
    }

    const size_t count_base = static_cast<size_t>(stone_count_param_id) * cfg.dim;
    for (int d = 0; d < cfg.dim; ++d) {
        const double count_vec = count_vectors[count_base + d];
        const double count_grad =
            -residual * cfg.count_weight * count_scratch.pattern_sums[d] +
            cfg.l2 * count_vec;
        const double next_count_m = 0.9 * count_m[count_base + d] + 0.1 * count_grad;
        const double next_count_v = 0.999 * count_v[count_base + d] + 0.001 * count_grad * count_grad;
        double next_count_vec = count_vec - lr_t * next_count_m / (std::sqrt(next_count_v) + 1.0e-8);
        next_count_vec *= decay;
        count_m[count_base + d] = static_cast<float>(next_count_m);
        count_v[count_base + d] = static_cast<float>(next_count_v);
        count_vectors[count_base + d] = static_cast<float>(next_count_vec);
    }
}
        ...
        while (cfg.max_records_per_file <= 0 || n_file < static_cast<uint64_t>(cfg.max_records_per_file)) {
            ...
            for (const Record &record : chunk) {
                update_record(
                    ...
                    count_vectors, count_m, count_v, count_initialized,
...
PhaseResult train_phase(
    ...
    std::vector<float> &best_count_vectors_out
) {
    ...
    std::vector<float> count_vectors(static_cast<size_t>(N_PARAMS_PER_PHASE) * cfg.dim, 0.0f);
    std::vector<float> count_m(static_cast<size_t>(N_PARAMS_PER_PHASE) * cfg.dim, 0.0f);
    std::vector<float> count_v(static_cast<size_t>(N_PARAMS_PER_PHASE) * cfg.dim, 0.0f);
    std::vector<uint8_t> count_initialized(static_cast<size_t>(N_PARAMS_PER_PHASE), 0);
    std::vector<uint8_t> best_count_initialized(static_cast<size_t>(N_PARAMS_PER_PHASE), 0);

    load_initial_egev4_phase_vectors(cfg.init_cross_egev4, cfg, phase, cross_vectors, cross_initialized, "cross");
    load_initial_egev4_phase_vectors(cfg.init_same_egev4, cfg, phase, same_vectors, same_initialized, "same");
    load_initial_egev4_phase_vectors(cfg.init_count_egev4, cfg, phase, count_vectors, count_initialized, "count");
    ...
    EpochRow initial;
    initial.epoch = 0;
    initial.n_initialized = count_initialized_params(cross_initialized);
    initial.train = compute_metrics(
        data_root, phase, cfg.train_ids, cfg.train_metric_max_records_per_file,
        linear, cross_vectors, same_vectors, count_vectors, cfg
    );
    ...
    result.count_vector_scale = vector_scale_for(best_count_vectors_out, &best_count_initialized, cfg.dim, result.count_vector_max_abs);
    ...
    result.count_nonzero_vector_values = count_quantized_nonzero(
        best_count_vectors_out, best_count_initialized, cfg.dim, result.count_vector_scale
    );
    return result;
}
...
void write_summary(
    ...
    out << "optimizer=current-linear-fixed joint-tri-type-FM streaming Adam finetune-from-EGEV4\n";
    out << "fm_interaction_features=board-pattern features plus stone-count-to-pattern pairs\n";
    out << "fm_definition=linear + cross_weight * cross_type_FM + same_weight * same_type_FM + count_weight * count_pattern_FM\n";
    ...
    out << "count_pattern_FM=pairs between the stone-count feature and all 64 board-pattern features\n";
    ...
    out << "init_cross_egev4=" << cfg.init_cross_egev4 << "\n";
    out << "init_same_egev4=" << cfg.init_same_egev4 << "\n";
    out << "init_count_egev4=" << cfg.init_count_egev4 << "\n";
    ...
    out << "output_count_egev4=" << cfg.output_count_egev4 << "\n";
    out << "eval_spec=" << cfg.output_cross_egev4 << '@' << cfg.output_same_egev4
        << '@' << cfg.output_count_egev4 << '@'
...
int main(int argc, char **argv) {
    try {
        ...
        std::map<int, std::vector<float>> phase_count_vectors;
        std::map<int, PhaseResult> phase_results;
        for (int phase : cfg.phases) {
            std::vector<float> best_cross_vectors;
            std::vector<float> best_same_vectors;
            std::vector<float> best_count_vectors;
            PhaseResult result = train_phase(cfg, data_root, phase, linear_values, best_cross_vectors, best_same_vectors, best_count_vectors);
            phase_cross_vectors[phase] = std::move(best_cross_vectors);
            phase_same_vectors[phase] = std::move(best_same_vectors);
            phase_count_vectors[phase] = std::move(best_count_vectors);
        ...
        write_egev4(cfg, linear_values, phase_count_vectors, phase_results, cfg.output_count_egev4, 2);
        ...
        std::cout << "wrote_count " << cfg.output_count_egev4 << std::endl;

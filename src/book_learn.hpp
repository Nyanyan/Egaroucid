...
int book_learn_search(Board board, int level, const int book_depth, const int best_value, const int expected_error, Board *board_copy, uint64_t *strt_tim, string book_file, string book_bak){
    if (!global_searching)
    ...
    if (board.n >= 4 + book_depth){
        ...
        if (g <= best_value || best_value < -SCORE_MAX || SCORE_MAX < best_value){
    ...
    if (legal == 0ULL){
        board.pass();
            g = -book_learn_search(board, level, book_depth, -best_value, expected_error, board_copy, strt_tim, book_file, book_bak);
        board.pass();
        return g;
    }
    ...
    board.move(&flip);
        ...
        g = -book_learn_search(board, level, book_depth, SCORE_UNDEFINED, expected_error, board_copy, strt_tim, book_file, book_bak);
    ...
    for (uint_fast8_t cell = first_bit(&legal); legal; cell = next_bit(&legal)){
        ...
        board.move(&flip);
            ...
            if (flag || (-SCORE_MAX <= g && g <= SCORE_MAX && g >= best_move.value - expected_error)){
                g = -book_learn_search(board, level, book_depth, -best_move.value, expected_error, board_copy, strt_tim, book_file, book_bak);
                if (-SCORE_MAX <= g && g <= SCORE_MAX){
...
inline void learn_book(Board root_board, int level, const int book_depth, const int expected_error, Board *board_copy, string book_file, string book_bak, bool *book_learning){
    ...
    int g = book_learn_search(root_board, level, book_depth, SCORE_UNDEFINED, expected_error, board_copy, &strt_tim, book_file, book_bak);

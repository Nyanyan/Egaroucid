...
*/
Parallel_task ybwc_do_task_nws(uint64_t player, uint64_t opponent, int_fast8_t n_discs, uint_fast8_t parity, uint_fast8_t mpc_level, bool is_presearch, int parent_alpha, const int depth, uint64_t legal, const bool is_end_search, uint_fast8_t policy, int move_idx, const bool *searching, bool *searching2) {
    ...
    Parallel_task task;
    task.value = -nega_alpha_ordering_nws(&search, -parent_alpha - 1, depth, false, legal, is_end_search, searching, searching2);
...
*/
inline int ybwc_split_nws(Search *search, int parent_alpha, const int depth, uint64_t legal, const bool is_end_search, const bool *searching, bool *searching2, uint_fast8_t policy, const int n_remaining_moves, const int move_idx, const int running_count, std::vector<std::future<Parallel_task>> &parallel_tasks) {
    if (
        ...
        if (!is_end_search && search->mpc_level < MPC_100_LEVEL) {
            if (mpc(search, -parent_alpha - 1, -parent_alpha, depth, legal, is_end_search, &v, searching, searching2)) {
                return -v;
            }
        }
        if (*searching) {
            bool pushed;
            parallel_tasks.emplace_back(thread_pool.push(&pushed, std::bind(&ybwc_do_task_nws, search->board.player, search->board.opponent, search->n_discs, search->parity, search->mpc_level, search->is_presearch, parent_alpha, depth, legal, is_end_search, policy, move_idx, searching, searching2)));
            if (pushed) {
                return YBWC_PUSHED;
            } else{
...
#if USE_YBWC_NWS
    inline void ybwc_search_young_brothers_nws(Search *search, int alpha, int *v, int *best_move, int n_available_moves, uint32_t hash_code, int depth, bool is_end_search, std::vector<Flip_value> &move_list, const bool *searching) {
        ...
        for (int move_idx = 1; move_idx < canput && n_searching && *searching; ++move_idx) {
            if (move_list[move_idx].flip.flip) {
                ...
                search->move(&move_list[move_idx].flip);
                    int ybwc_split_state = ybwc_split_nws(search, alpha, depth - 1, move_list[move_idx].n_legal, is_end_search, searching, &n_searching, move_list[move_idx].flip.pos, n_available_moves - n_moves_seen, move_idx, running_count, parallel_tasks);
            ...
            if (running_count) {
                ...
                if (*v <= alpha && n_to_be_searched && *searching) {
                    if (n_to_be_searched <= 1) {
                        ...
                        ybwc_search_young_brothers_nws(search, alpha, v, best_move, n_to_be_searched, hash_code, depth, is_end_search, move_list, searching);
                    }
                    return;
                }
            }

/*
    ...
    @file data_board_to_idx_multi_phase_experiment_edax_linear_engine_coords.cpp
        One-pass board-data converter for the isolated Edax-linear experiment,
        using the same coordinate convention as the engine evaluator
    @date 2026
...
inline int edax_pick_pattern_engine_coords(const uint_fast8_t b_arr[], int pattern_idx) {
    int res = 0;
...
inline void edax_calc_engine_coord_features(Board *board, uint16_t res[]) {
    uint_fast8_t b_arr[HW2];
    board->translate_to_arr_player(b_arr);
    for (int i = 0; i < ADJ_N_FEATURES; ++i) {
        res[i] = (uint16_t)edax_pick_pattern_engine_coords(b_arr, i);
    }
}

int main(int argc, char *argv[]) {
    std::cerr << EVAL_DEFINITION_NAME << "_engine_coords" << std::endl;
    std::cerr << EVAL_DEFINITION_DESCRIPTION << " with engine-coordinate feature IDs" << std::endl;
    if (argc < 10) {
    ...
    for (int file_idx = start_file; file_idx < end_file; ++file_idx) {
        ...
        while (!all_phase_full()) {
            ...
            edax_calc_engine_coord_features(&board, idxes);

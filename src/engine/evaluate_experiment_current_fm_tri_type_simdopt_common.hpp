...
template<bool UpdateTotal>
inline void current_fm_add_vector_simd4(
    const CurrentFmTriFile &src,
    const int phase_idx,
    const int id,
    CurrentFmSimdAccum &accum,
    ...
    const int8_t *src_ptr = phase_vectors.data() + (size_t)id * 4;
    int32_t packed = 0;
    std::memcpy(&packed, src_ptr, sizeof(packed));
...
inline void current_fm_load_vector_simd12(
    const CurrentFmTriFile &src,
    const int phase_idx,
    const int id,
    __m256i &lo,
...
inline int current_fm_score_from_ids_simd12(const int phase_idx, const int active_ids[], const int n_active) {
    if (!current_fm_loaded || phase_idx < 0 || N_PHASES <= phase_idx) {
        return 0;
    }
    const size_t phase_base = (size_t)phase_idx * CURRENT_FM_N_PARAMS_PER_PHASE;
    ...
    for (int i = 0; i < n_active; ++i) {
        ...
        const int pattern_type = current_fm_pattern_type_for_active(i, id);
        if (pattern_type >= 0) {
            current_fm_add_vector_simd12<true>(current_fm_cross_file, phase_idx, id, cross_accum, pattern_type);
            current_fm_add_vector_simd12<false>(current_fm_same_file, phase_idx, id, same_accum, pattern_type);
            __m256i pattern_lo = _mm256_setzero_si256();
            __m256i pattern_hi = _mm256_setzero_si256();
            current_fm_load_vector_simd12(current_fm_count_file, phase_idx, id, pattern_lo, pattern_hi);
            count_pattern_sum_lo = _mm256_add_epi32(count_pattern_sum_lo, pattern_lo);
            count_pattern_sum_hi = _mm256_add_epi32(count_pattern_sum_hi, pattern_hi);
        } else if (id >= CURRENT_FM_N_PATTERN_PARAMS_RAW) {
            current_fm_load_vector_simd12(current_fm_count_file, phase_idx, id, count_vec_lo, count_vec_hi);
        }
    }
...
inline int current_fm_score_from_ids_simd4(const int phase_idx, const int active_ids[], const int n_active) {
    if (!current_fm_loaded || phase_idx < 0 || N_PHASES <= phase_idx) {
        return 0;
    }
    const size_t phase_base = (size_t)phase_idx * CURRENT_FM_N_PARAMS_PER_PHASE;
    ...
    for (int i = 0; i < n_active; ++i) {
        ...
        const int pattern_type = current_fm_pattern_type_for_active(i, id);
        if (pattern_type >= 0) {
            current_fm_add_vector_simd4<true>(current_fm_cross_file, phase_idx, id, cross_accum, pattern_type);
            current_fm_add_vector_simd4<false>(current_fm_same_file, phase_idx, id, same_accum, pattern_type);
            __m256i pattern_lo = _mm256_setzero_si256();
            __m256i pattern_hi = _mm256_setzero_si256();
            current_fm_load_vector_simd4(current_fm_count_file, phase_idx, id, pattern_lo, pattern_hi);
            count_pattern_sum_lo = _mm256_add_epi32(count_pattern_sum_lo, pattern_lo);
            count_pattern_sum_hi = _mm256_add_epi32(count_pattern_sum_hi, pattern_hi);
        } else if (id >= CURRENT_FM_N_PATTERN_PARAMS_RAW) {
            current_fm_load_vector_simd4(current_fm_count_file, phase_idx, id, count_vec_lo, count_vec_hi);
        }
    }
...
inline int current_fm_score_from_ids(const int phase_idx, const int active_ids[], const int n_active) {
    ...
    if (current_fm_cross_file.dim == 12 && current_fm_same_file.dim == 12 && current_fm_count_file.dim == 12) {
        return current_fm_score_from_ids_simd12(phase_idx, active_ids, n_active);
    }

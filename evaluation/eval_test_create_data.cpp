...
inline void output_board(unsigned long long p, unsigned long long o, int value){
    value += HW2;
    for (int i = 0; i < HW2; ++i){
        if (1 & (p >> i)){
            cout << '0';
    ...
    cout << value / 100 << (value / 10) % 10 << value % 10 << endl;
}
...
inline bool import_edax_book(string file) {
    ...
    for (i = 0; i < n_boards; ++i){
        ...
        output_board(player, opponent, -(int)value);

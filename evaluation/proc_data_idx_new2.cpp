...
constexpr Joined_pattern joined_pattern[N_SYMMETRY_PATTERNS] = {
    ...
    {2, {0x0080808080800000ULL, 0x000000000000003EULL, 0x0000000000000000ULL}}, // 54
    {2, {0x000000000000007CULL, 0x0001010101010000ULL, 0x0000000000000000ULL}}, // 55
    {2, {0x0000010101010100ULL, 0x7C00000000000000ULL, 0x0000000000000000ULL}}, // 56
    {2, {0x3E00000000000000ULL, 0x0000808080808000ULL, 0x0000000000000000ULL}}, // 57

    {2, {0x003C000000000000ULL, 0x3C00000000000000ULL, 0x0000000000000000ULL}}, // 58
    {2, {0x0000000000003C00ULL, 0x000000000000003CULL, 0x0000000000000000ULL}}, // 59
    {2, {0x0000404040400000ULL, 0x0000808080800000ULL, 0x0000000000000000ULL}}, // 60
    {2, {0x0000020202020000ULL, 0x0000010101010000ULL, 0x0000000000000000ULL}}, // 61

    {2, {0x0000808080808080ULL, 0x3F00000000000000ULL, 0x0000000000000000ULL}}, // 62
    {2, {0x8080808080800000ULL, 0x000000000000003FULL, 0x0000000000000000ULL}}, // 63
    {2, {0x0000010101010101ULL, 0xFC00000000000000ULL, 0x0000000000000000ULL}}, // 64
    {2, {0x0101010101010000ULL, 0x00000000000000FCULL, 0x0000000000000000ULL}}, // 65

    {3, {0x0000808080808080ULL, 0x0000201008040201ULL, 0x3F00000000000000ULL}}, // 66
    {3, {0x8080808080800000ULL, 0x0102040810200000ULL, 0x000000000000003FULL}}, // 67
    {3, {0x0000010101010101ULL, 0x0000040810204080ULL, 0xFC00000000000000ULL}}, // 68
    {3, {0x0101010101010000ULL, 0x8040201008040000ULL, 0x00000000000000FCULL}}  // 69
};
...
inline void calc_idx(int phase_idx, Board *b, int idxes[]){
    ...
    for (i = 0; i < N_SYMMETRY_PATTERNS; ++i){
        if (feature_to_coord[i].n_cells == 10)
            ...
            cerr << "err" << endl;
        if (joined_pattern[i].n_joined == 1){
            idxes[i] *= 8;
            idxes[i] += pick_joined_pattern(b, joined_pattern[i].mask[0]);
        } else if (joined_pattern[i].n_joined == 2){
            idxes[i] *= 64;
            idxes[i] += pick_joined_pattern(b, joined_pattern[i].mask[0]) * 8;
            idxes[i] += pick_joined_pattern(b, joined_pattern[i].mask[1]);
        } else if (joined_pattern[i].n_joined == 3){
            idxes[i] *= 512;
            idxes[i] += pick_joined_pattern(b, joined_pattern[i].mask[0]) * 64;
            idxes[i] += pick_joined_pattern(b, joined_pattern[i].mask[1]) * 8;

/*
    ...
    Current-linear fixed same-type FM optimizer with streaming chunk
    ...
    interaction and vector updates use only pairs among the four orientations
    of the same pattern type.  Interactions between different pattern types and
    interactions with the stone-count feature are excluded from the FM term.
*/
...
double predict_record(
    ...
    for (int d = 0; d < dim; ++d) {
        for (int pattern_type = 0; pattern_type < N_PATTERN_TYPES; ++pattern_type) {
            ...
            interaction += group_sums[group_idx] * group_sums[group_idx] - group_sums_sq[group_idx];
        }
    }
...
void update_record(
    ...
    for (int feature_idx = 0; feature_idx < N_PATTERN_FEATURES; ++feature_idx) {
        ...
        for (int d = 0; d < cfg.dim; ++d) {
            const double vec = vectors[base + d];
            const double grad = -residual * (group_sums[group_base + d] - vec) + cfg.l2 * vec;
...
void write_summary(
    ...
    out << "optimizer=current-linear-fixed same-type-FM streaming Adam\n";
    out << "fm_interaction_features=board-pattern features only, same pattern type only\n";

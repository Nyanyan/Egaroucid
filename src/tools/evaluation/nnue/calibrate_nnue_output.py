#!/usr/bin/env python3
"""Calibrate NNUE output heads with linear per-phase correction."""
...
from analyze_nnue_predictions import mae_mse, predict_float, summarize_by_phase
...
def fit_phase_calibration(
    ...
    for phase in range(60):
        ...
        if var_x <= 1.0e-12:
            ...
            slope = float(np.cov(x, y, bias=True)[0, 1] / var_x)
...
def apply_calibration(model: OthelloNNUE, slopes: np.ndarray, offsets: np.ndarray) -> None:
    with torch.no_grad():
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model-name", default="nnue_small_output_calibrated_train1m_val1m_q32")
...
def main() -> int:
    ...
    out_file = Path(args.out_file) if args.out_file else out_dir / f"eval_nnue_{args.arch}_calibrated.egevnnue"
    ...
    summary = {
        "method": "per_phase_output_linear_calibration",

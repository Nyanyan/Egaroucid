...
public:
    Update_check(const InitData& init) : IScene{ init } {
        ...
        back_button.init(BACK_BUTTON_SX, BACK_BUTTON_SY, BACK_BUTTON_WIDTH, BACK_BUTTON_HEIGHT, BACK_BUTTON_RADIUS, language.get("loading", "launch"), 25, getData().fonts.font, getData().colors.white, getData().colors.black);
        ...
        if (update_found) {
            ...
            if (check_future.valid()) {
                if (check_future.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
                    update_found = check_future.get() == UPDATE_CHECK_UPDATE_FOUND;
                }
            }

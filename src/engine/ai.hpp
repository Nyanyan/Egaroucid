...
Search_result ai_time_limit(Board board, bool use_book, int book_acc_level, bool use_multi_thread, bool show_log, uint64_t remaining_time_msec, thread_id_t thread_id, bool *searching) {
    ...
    if (time_limit > 10000ULL && n_empties >= 33) { // additional search
        ...
        if (second_move_list.size()) {
            ...
            int n_good_moves = 0;
            for (const Ponder_elem &elem: second_move_list) {
            ...
            if (n_good_moves >= 2) {
                if (show_log) {
                    ...
                    for (int i = 0; i < n_good_moves; ++i) {
                        std::cerr << " " << idx_to_coord(second_move_list[i].flip.pos);
                    }
                    std::cerr << std::endl;
                }
                uint64_t elapsed_till_second_search = tim() - strt;
    ...
    while (tim() - strt < time_limit) {
        ...
        for (int i = n_boards.size() - 1; i >= 0; --i) {
            ...
            if (elapsed < time_limit) {
                ...
                if (!is_first_searches[selected_idx] && move_list[selected_idx].depth < 31) { // limit root depth min(32, move_list[selected_idx].depth)

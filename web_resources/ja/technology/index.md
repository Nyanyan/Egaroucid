...
あるノードを[α,β]の探索窓で探索するとします。また、子ノードを展開して、各子ノードについて置換表に記載されている(子ノードの手番から見た)最小値Lと最大値Hを見ていきます。すると、β<max({-L})であればβをmax({-L})に更新できますし、同様にしてα>max({-H})であればαをmax({-H})に更新できます。
...
しかし、当然ながら評価関数は完璧ではありません。例えば大量取りという状況は、大量取りしている方が圧倒的に不利になりやすいのですが、たまにそのまま大量取りをした方が相手を全滅させるなどできる場合があります。このような場合は評価関数が苦手なことが多く、評価関数による探索だけに頼ると全滅筋にはまってしまう場合もあります。

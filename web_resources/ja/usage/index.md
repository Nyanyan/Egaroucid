...
バージョン7.8.2からは、うみがめ数の計算条件として黒目線のスコア範囲と累積石損を設定できます。スコア範囲外の手やbookにない手は空欄になり、条件に合う進行だけを対象にしてうみがめ数を計算します。累積石損を0にすると、各局面で局所的な最善手だけを辿る従来に近い挙動になります。設定は変更後に自動で反映されます。
...
<div class="centering_box">
    ...
    <img class="pic2" src="img/timer_controls.png">
    <img class="pic2" src="img/timer_time_limit_setting.png">
</div>
...
バージョン7.8.2からは、ランダム局面生成やXOT局面生成の途中に作られる局面について、通常の対局グラフに値を表示しないようにしました。生成された局面そのものからの対局や分析に集中しやすくなります。
...
指定された手数までの最後の2手は、指定されたスコア範囲に収まるような手があればその手を、なければスコア範囲に近い手を選ぶようにしています。このスコア範囲への調整には<a href="#設定_レベル">設定 > レベル</a>で設定したAIレベルを使用します。最後から2手目はレベル15、最後の1手は設定したAIレベルで探索します。ただし、設定したAIレベルが15未満の場合、最後から2手目も設定したAIレベルまで下げて探索します。また、通常部分のレベル1も設定したAIレベルを超えないようにしています。
...
バージョン7.7.0までは似た機能でEdax準拠のBook修正がありましたが、これは7.8.0で廃止しました。その代わり、Edax形式のBookを読み込むときの挙動を変更し、通常のBook修正でEdaxのBook Fixと同様の処理が行われるようにしました。しかし、完全な互換性は担保しません。
...
バージョン7.8.0で、Edax形式のBookを書き出すときの処理を一部変更しました。

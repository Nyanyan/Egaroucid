...
<ul>
    <li>対局
        <ul>
            <li><a href="#対局_新規対局 (人間同士)">新規対局 (人間同士)</a> N</li>
        </ul>
    </li>
    <li>設定
        <ul>
            <li><a href="#設定_AIの着手設定">AIが黒に着手</a> B</li>
            <li><a href="#設定_AIの着手設定">AIが白に着手</a> W</li>
        </ul>
    </li>
    <li>表示
        <ul>
            <li><a href="#表示_合法手への表示 > 石差評価">合法手への表示 > 石差評価</a> V</li>
            <li><a href="#表示_合法手への表示 > うみがめ数">合法手への表示 > うみがめ数</a> U</li>
        </ul>
    </li>
</ul>

基本操作として、「取り込む」や「出力する」「スタート」「OK」のようなニュアンスのボタンはEnterキーで操作できます。また、「戻る」や「この画面から抜ける」のようなニュアンスのボタンはEscキーで操作できます。
...
カーソル移動ができないので、特にメモはコピペを推奨します。ペーストはCtrl+Vで行ってください。
...
手筋の誤差$E_{sum}$は、bookの学習を開始した局面の評価値を$V$として、$[V-E_{sum},V+E_{sum}]$までの評価値を持つ局面に絞って探索するという意味です。

つまり、$D$手目以上の局面は登録されませんし、$E_1$石以上の石損をする手は登録されません($E_1$石損以下だと思ったが、計算してみたら$E_1$石損以上であった、という場合には登録されます)し、石損を繰り返して最初の局面から$E_\mathrm{sum}$以上評価値が離れた手は登録されません。
...
学習を開始したい開始局面を作り、使用するAIのレベルとbookの学習設定を確認し、そこでこのボタンを押すと確認画面に移行します。
...
<a href="#Book_Bookの設定 (深さ、1手あたりの誤差、手筋の誤差)">bookの設定</a>で設定した項目に従って、指定局面から先の局面に対して、あまりにも悪い手や、設定した深さよりも先の局面がbookに登録されていたら登録を解除します。
...
この機能では、<a href="#設定_レベル">設定>レベル</a>で設定できるAIのレベルと、<a href="#Book_Bookの設定 (深さ、1手あたりの誤差、手筋の誤差)">book設定</a>の項目で設定できる各項目が使用されます。
...
言語パックはEgaroucidインストールフォルダの中のresources/languagesの中にある言語名.jsonです。ここに言語パックを追加して、languages.jsonに言語の英名と言語の表示名を書けば、Egaroucidに反映されます。

/*
    ...
    Current-linear fixed joint tri-type FM optimizer with pattern-type count
    interaction, streaming chunk shuffling, and pseudo-Huber loss experiment.
    ...
    feature, including the stone-count feature.  Three independent FM vector
    ...
    - count-pattern-by-type bank: pairs between the stone-count feature and
      board pattern features, with one stone-count vector per pattern type

    The tool writes two EGEV4 files for cross/same vector banks and one EGCT
    file for the pattern-type count bank.
*/
...
constexpr int VERSION_COUNT_BY_TYPE_INT8 = 1;
...
constexpr uint64_t FM_BANK_SALT_COUNT_PATTERN = 0x434e545f50415454ULL; // "CNT_PATT"
constexpr uint64_t FM_BANK_SALT_COUNT_TYPE = 0x434e545f54595045ULL;    // "CNT_TYPE"
...
struct Config {
    ...
    std::string output_count_type_egct;
    std::string summary;
...
struct PhaseResult {
    ...
    double count_vector_scale = 1.0;
    double count_vector_max_abs = 0.0;
    uint64_t count_nonzero_vector_values = 0;
    uint64_t count_pattern_nonzero_vector_values = 0;
    uint64_t count_type_nonzero_vector_values = 0;
...
[[noreturn]] void usage_error(const std::string &message) {
    throw std::runtime_error(message + "\n"
        ...
        "--output-cross-egev4 FILE --output-same-egev4 FILE --output-count-type-egct FILE --summary FILE [options]");
}
...
Config parse_args(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--output-count-type-egct" || arg == "--output-count-egct") {
            cfg.output_count_type_egct = require_value(i, argc, argv);
        } else if (arg == "--summary") {
        ...
        } else if (arg == "--count-weight") {
            cfg.count_weight = std::stod(require_value(i, argc, argv));
    ...
    if (cfg.output_count_type_egct.empty()) {
        usage_error("--output-count-type-egct is required");
    }
    ...
    if (!std::isfinite(cfg.cross_weight) || !std::isfinite(cfg.same_weight) || !std::isfinite(cfg.count_weight)) {
        usage_error("--cross-weight, --same-weight, and --count-weight must be finite");
    }
...
void initialize_count_type_param(
    int stone_count,
    int pattern_type,
    int phase,
    const Config &cfg,
    ...
    const int param_id = stone_count * N_PATTERN_TYPES + pattern_type;
    if (param_id < 0 || static_cast<size_t>(param_id) >= initialized.size()) {
        return;
    }
    if (initialized[static_cast<size_t>(param_id)] != 0) {
        return;
    }
    const size_t base = static_cast<size_t>(param_id) * cfg.dim;
    for (int d = 0; d < cfg.dim; ++d) {
        vectors[base + d] = deterministic_init(cfg.seed, phase, static_cast<uint32_t>(param_id), d, FM_BANK_SALT_COUNT_TYPE, cfg.init_scale);
        m[base + d] = 0.0f;
        v[base + d] = 0.0f;
    }
    initialized[static_cast<size_t>(param_id)] = 1;
}
...
struct CountByTypeInteractionScratch {
    std::vector<double> pattern_type_sums;
    std::vector<double> count_type_vecs;

    explicit CountByTypeInteractionScratch(int dim)
        : pattern_type_sums(static_cast<size_t>(N_PATTERN_TYPES * dim), 0.0),
          count_type_vecs(static_cast<size_t>(N_PATTERN_TYPES * dim), 0.0) {}

    void clear() {
        std::fill(pattern_type_sums.begin(), pattern_type_sums.end(), 0.0);
        std::fill(count_type_vecs.begin(), count_type_vecs.end(), 0.0);
    }
};
...
double predict_record(
    ...
    const std::vector<float> &count_pattern_vectors,
    const std::vector<float> &count_type_vectors,
    const Record &record,
    int dim,
    double cross_weight,
    double same_weight,
    ...
    CountByTypeInteractionScratch &count_scratch
) {
    cross_scratch.clear();
    same_scratch.clear();
    count_scratch.clear();
    ...
    for (int feature_idx = 0; feature_idx < N_FEATURES; ++feature_idx) {
        ...
        if (pattern_type < 0) {
            const int stone_count = static_cast<int>(param_id) - N_PATTERN_PARAMS_RAW;
            if (0 <= stone_count && stone_count < MAX_STONE_NUM) {
                const size_t count_base = static_cast<size_t>(stone_count) * N_PATTERN_TYPES * dim;
                for (int pattern_type_idx = 0; pattern_type_idx < N_PATTERN_TYPES; ++pattern_type_idx) {
                    const size_t group_base = static_cast<size_t>(pattern_type_idx) * dim;
                    const size_t vec_base = count_base + group_base;
                    for (int d = 0; d < dim; ++d) {
                        count_scratch.count_type_vecs[group_base + d] = count_type_vectors[vec_base + d];
                    }
                }
            }
            continue;
        ...
        for (int d = 0; d < dim; ++d) {
            ...
            count_scratch.pattern_type_sums[group_base + d] += count_pattern_vectors[base + d];
        }
    }

    double cross_interaction = 0.0;
    double same_interaction = 0.0;
    double count_interaction = 0.0;
    for (int d = 0; d < dim; ++d) {
        ...
        for (int pattern_type = 0; pattern_type < N_PATTERN_TYPES; ++pattern_type) {
            const size_t group_idx = static_cast<size_t>(pattern_type) * dim + d;
            count_interaction += count_scratch.pattern_type_sums[group_idx] * count_scratch.count_type_vecs[group_idx];
        }
    }
    return value
        + 0.5 * cross_weight * cross_interaction
        + 0.5 * same_weight * same_interaction
        + count_weight * count_interaction;
}
...
Metrics compute_metrics(
    ...
    CountByTypeInteractionScratch count_scratch(cfg.dim);
    for (int file_id : file_ids) {
        const fs::path path = data_path(root, phase, file_id);
        std::ifstream in(path, std::ios::binary);
        if (!in) {
        ...
        while (max_records_per_file <= 0 || n_file < static_cast<uint64_t>(max_records_per_file)) {
            ...
            const double pred = predict_record(
                linear, cross_vectors, same_vectors, count_pattern_vectors, count_type_vectors, record, cfg.dim,
                cfg.cross_weight, cfg.same_weight, cfg.count_weight, cross_scratch, same_scratch, count_scratch
            );
...
uint64_t count_initialized_params(const std::vector<uint8_t> &initialized) {
    uint64_t n = 0;
...
void update_record(
    ...
    std::vector<float> &count_pattern_vectors,
    std::vector<float> &count_pattern_m,
    std::vector<float> &count_pattern_v,
    std::vector<uint8_t> &count_pattern_initialized,
    std::vector<float> &count_type_vectors,
    std::vector<float> &count_type_m,
    std::vector<float> &count_type_v,
    std::vector<uint8_t> &count_type_initialized,
    uint64_t &step,
    double &beta1_power,
    double &beta2_power,
    InteractionScratch &cross_scratch,
    ...
    for (int feature_idx = 0; feature_idx < N_PATTERN_FEATURES; ++feature_idx) {
        ...
        initialize_param(param_id, phase, cfg, FM_BANK_SALT_COUNT_PATTERN, count_pattern_vectors, count_pattern_m, count_pattern_v, count_pattern_initialized);
    }
    const uint32_t stone_count_param_id = record.active_ids[N_PATTERN_FEATURES];
    const int stone_count = static_cast<int>(stone_count_param_id) - N_PATTERN_PARAMS_RAW;
    for (int pattern_type = 0; pattern_type < N_PATTERN_TYPES; ++pattern_type) {
        initialize_count_type_param(
            stone_count, pattern_type, phase, cfg,
            count_type_vectors, count_type_m, count_type_v, count_type_initialized
        );
    }
    ...
    for (int feature_idx = 0; feature_idx < N_PATTERN_FEATURES; ++feature_idx) {
        ...
        for (int d = 0; d < cfg.dim; ++d) {
            ...
            const double count_vec = count_pattern_vectors[base + d];
            const double count_grad =
                -residual * cfg.count_weight * count_scratch.count_type_vecs[group_base + d] +
                cfg.l2 * count_vec;
            const double next_count_m = 0.9 * count_pattern_m[base + d] + 0.1 * count_grad;
            const double next_count_v = 0.999 * count_pattern_v[base + d] + 0.001 * count_grad * count_grad;
            double next_count_vec = count_vec - lr_t * next_count_m / (std::sqrt(next_count_v) + 1.0e-8);
            next_count_vec *= decay;
            count_pattern_m[base + d] = static_cast<float>(next_count_m);
            count_pattern_v[base + d] = static_cast<float>(next_count_v);
            count_pattern_vectors[base + d] = static_cast<float>(next_count_vec);
        }
    }

    for (int pattern_type = 0; pattern_type < N_PATTERN_TYPES; ++pattern_type) {
        const size_t group_base = static_cast<size_t>(pattern_type) * cfg.dim;
        const size_t count_base = (static_cast<size_t>(stone_count) * N_PATTERN_TYPES + pattern_type) * cfg.dim;
        for (int d = 0; d < cfg.dim; ++d) {
            const double count_vec = count_type_vectors[count_base + d];
            const double count_grad =
                -residual * cfg.count_weight * count_scratch.pattern_type_sums[group_base + d] +
                cfg.l2 * count_vec;
            const double next_count_m = 0.9 * count_type_m[count_base + d] + 0.1 * count_grad;
            const double next_count_v = 0.999 * count_type_v[count_base + d] + 0.001 * count_grad * count_grad;
            double next_count_vec = count_vec - lr_t * next_count_m / (std::sqrt(next_count_v) + 1.0e-8);
            next_count_vec *= decay;
            count_type_m[count_base + d] = static_cast<float>(next_count_m);
            count_type_v[count_base + d] = static_cast<float>(next_count_v);
            count_type_vectors[count_base + d] = static_cast<float>(next_count_vec);
        }
    }
}
        ...
        while (cfg.max_records_per_file <= 0 || n_file < static_cast<uint64_t>(cfg.max_records_per_file)) {
            ...
            for (const Record &record : chunk) {
                update_record(
                    ...
                    count_pattern_vectors, count_pattern_m, count_pattern_v, count_pattern_initialized,
                    count_type_vectors, count_type_m, count_type_v, count_type_initialized,
                    step, beta1_power, beta2_power, cross_scratch, same_scratch, count_scratch
                );
                ++updates;
            }
            ++chunk_idx;
...
double vector_scale_for(
    const std::vector<float> &vectors,
    const std::vector<uint8_t> *initialized,
    int dim,
    double &max_abs,
    int n_params = N_PARAMS_PER_PHASE
) {
    max_abs = 0.0;
...
PhaseResult train_phase(
    ...
    std::vector<float> &best_count_pattern_vectors_out,
    std::vector<float> &best_count_type_vectors_out
) {
    ...
    std::vector<float> count_pattern_vectors(static_cast<size_t>(N_PATTERN_PARAMS_RAW) * cfg.dim, 0.0f);
    std::vector<float> count_pattern_m(static_cast<size_t>(N_PATTERN_PARAMS_RAW) * cfg.dim, 0.0f);
    std::vector<float> count_pattern_v(static_cast<size_t>(N_PATTERN_PARAMS_RAW) * cfg.dim, 0.0f);
    std::vector<uint8_t> count_pattern_initialized(static_cast<size_t>(N_PATTERN_PARAMS_RAW), 0);
    std::vector<uint8_t> best_count_pattern_initialized(static_cast<size_t>(N_PATTERN_PARAMS_RAW), 0);

    std::vector<float> count_type_vectors(static_cast<size_t>(MAX_STONE_NUM * N_PATTERN_TYPES) * cfg.dim, 0.0f);
    std::vector<float> count_type_m(static_cast<size_t>(MAX_STONE_NUM * N_PATTERN_TYPES) * cfg.dim, 0.0f);
    std::vector<float> count_type_v(static_cast<size_t>(MAX_STONE_NUM * N_PATTERN_TYPES) * cfg.dim, 0.0f);
    std::vector<uint8_t> count_type_initialized(static_cast<size_t>(MAX_STONE_NUM * N_PATTERN_TYPES), 0);
    std::vector<uint8_t> best_count_type_initialized(static_cast<size_t>(MAX_STONE_NUM * N_PATTERN_TYPES), 0);
    ...
    initial.train = compute_metrics(
        ...
        linear, cross_vectors, same_vectors, count_pattern_vectors, count_type_vectors, cfg
    );
    ...
    best_count_pattern_vectors_out.assign(static_cast<size_t>(N_PATTERN_PARAMS_RAW) * cfg.dim, 0.0f);
    best_count_type_vectors_out.assign(static_cast<size_t>(MAX_STONE_NUM * N_PATTERN_TYPES) * cfg.dim, 0.0f);
    result.best_epoch = 0;
    ...
    for (int epoch = 1; epoch <= cfg.epochs; ++epoch) {
        ...
        EpochRow row;
        row.epoch = epoch;
        row.n_initialized = count_initialized_params(cross_initialized);
        ...
        if (row.holdout.mae < best_holdout_mae) {
            ...
            best_count_pattern_vectors_out = count_pattern_vectors;
            best_count_pattern_initialized = count_pattern_initialized;
            best_count_type_vectors_out = count_type_vectors;
            best_count_type_initialized = count_type_initialized;
    ...
    double count_pattern_max_abs = 0.0;
    double count_type_max_abs = 0.0;
    (void)vector_scale_for(
        best_count_pattern_vectors_out, &best_count_pattern_initialized, cfg.dim,
        count_pattern_max_abs, N_PATTERN_PARAMS_RAW
    );
    (void)vector_scale_for(
        best_count_type_vectors_out, &best_count_type_initialized, cfg.dim,
        count_type_max_abs, MAX_STONE_NUM * N_PATTERN_TYPES
    );
    result.count_vector_max_abs = std::max(count_pattern_max_abs, count_type_max_abs);
    result.count_vector_scale = result.count_vector_max_abs > 0.0 ? result.count_vector_max_abs / 127.0 : 1.0;
    ...
    if (result.count_vector_scale <= 0.0) {
        result.count_vector_scale = 1.0;
    }
    ...
    result.count_pattern_nonzero_vector_values = count_quantized_nonzero(
        best_count_pattern_vectors_out, best_count_pattern_initialized,
        cfg.dim, result.count_vector_scale, N_PATTERN_PARAMS_RAW
    );
    result.count_type_nonzero_vector_values = count_quantized_nonzero(
        best_count_type_vectors_out, best_count_type_initialized,
        cfg.dim, result.count_vector_scale, MAX_STONE_NUM * N_PATTERN_TYPES
    );
    result.count_nonzero_vector_values =
        result.count_pattern_nonzero_vector_values + result.count_type_nonzero_vector_values;
    return result;
}
...
void write_egev4(
    ...
    for (int phase = 0; phase < N_PHASES; ++phase) {
        ...
        if (it != phase_results.end()) {
            if (bank_kind == 0) {
                vector_scale = it->second.cross_vector_scale;
            } else if (bank_kind == 1) {
                vector_scale = it->second.same_vector_scale;
            } else {
                vector_scale = it->second.count_vector_scale;
            }
        }
    ...
    for (int phase = 0; phase < N_PHASES; ++phase) {
        ...
        if (result_it != phase_results.end()) {
            if (bank_kind == 0) {
                vector_scale = result_it->second.cross_vector_scale;
            } else if (bank_kind == 1) {
                vector_scale = result_it->second.same_vector_scale;
            } else {
                vector_scale = result_it->second.count_vector_scale;
            }
        }
        const size_t phase_base = static_cast<size_t>(phase) * N_PARAMS_PER_PHASE;
...
void write_count_type_egct(
    const Config &cfg,
    const std::map<int, std::vector<float>> &phase_count_pattern_vectors,
    const std::map<int, std::vector<float>> &phase_count_type_vectors,
    const std::map<int, PhaseResult> &phase_results
) {
    fs::path output_path(cfg.output_count_type_egct);
    if (!output_path.parent_path().empty()) {
        fs::create_directories(output_path.parent_path());
    }
    std::ofstream out(output_path, std::ios::binary);
    ...
    out.write(timestamp.data(), 14);
    out.write("EGCT", 4);
    write_i32(out, VERSION_COUNT_BY_TYPE_INT8);
    write_i32(out, N_PHASES);
    write_i32(out, N_PATTERN_PARAMS_RAW);
    write_i32(out, MAX_STONE_NUM);
    write_i32(out, N_PATTERN_TYPES);
    write_i32(out, cfg.dim);

    for (int phase = 0; phase < N_PHASES; ++phase) {
        auto it = phase_results.find(phase);
        const double vector_scale = it == phase_results.end() ? 1.0 : it->second.count_vector_scale;
        write_f32(out, static_cast<float>(vector_scale));
    }

    for (int phase = 0; phase < N_PHASES; ++phase) {
        auto result_it = phase_results.find(phase);
        const double vector_scale = result_it == phase_results.end() ? 1.0 : result_it->second.count_vector_scale;

        auto pattern_it = phase_count_pattern_vectors.find(phase);
        const std::vector<float> *pattern_vectors =
            pattern_it == phase_count_pattern_vectors.end() ? nullptr : &pattern_it->second;
        write_quantized_vectors(out, pattern_vectors, N_PATTERN_PARAMS_RAW, cfg.dim, vector_scale);

        auto type_it = phase_count_type_vectors.find(phase);
        const std::vector<float> *type_vectors =
            type_it == phase_count_type_vectors.end() ? nullptr : &type_it->second;
        write_quantized_vectors(out, type_vectors, MAX_STONE_NUM * N_PATTERN_TYPES, cfg.dim, vector_scale);
    }
}
...
void write_summary(
    ...
    out << "optimizer=current-linear-fixed joint-tri-type-FM count-by-pattern-type streaming Adam\n";
    out << "fm_interaction_features=board-pattern features plus stone-count-to-pattern pairs\n";
    out << "fm_definition=linear + cross_weight * cross_type_FM + same_weight * same_type_FM + count_weight * count_by_pattern_type_FM\n";
    ...
    out << "count_by_pattern_type_FM=pairs between board-pattern features and stone-count vectors split by the 16 pattern types\n";
    ...
    out << "count_weight=" << cfg.count_weight << "\n";
    ...
    out << "output_count_type_egct=" << cfg.output_count_type_egct << "\n";
    out << "eval_spec=" << cfg.output_cross_egev4 << '@' << cfg.output_same_egev4
        << '@' << cfg.output_count_type_egct << '@'
        << cfg.cross_weight << '@' << cfg.same_weight << '@' << cfg.count_weight << "\n";
    ...
    for (const auto &[phase, result] : phase_results) {
        ...
        out << "count_vector_scale=" << result.count_vector_scale << "\n";
        out << "count_vector_max_abs=" << result.count_vector_max_abs << "\n";
        out << "count_nonzero_vector_values=" << result.count_nonzero_vector_values << "\n";
        out << "count_pattern_nonzero_vector_values=" << result.count_pattern_nonzero_vector_values << "\n";
        out << "count_type_nonzero_vector_values=" << result.count_type_nonzero_vector_values << "\n";
...
int main(int argc, char **argv) {
    try {
        ...
        std::map<int, std::vector<float>> phase_count_pattern_vectors;
        std::map<int, std::vector<float>> phase_count_type_vectors;
        std::map<int, PhaseResult> phase_results;
        for (int phase : cfg.phases) {
            std::vector<float> best_cross_vectors;
            std::vector<float> best_same_vectors;
            std::vector<float> best_count_pattern_vectors;
            std::vector<float> best_count_type_vectors;
            PhaseResult result = train_phase(
                cfg, data_root, phase, linear_values,
                best_cross_vectors, best_same_vectors,
                best_count_pattern_vectors, best_count_type_vectors
            );
            phase_cross_vectors[phase] = std::move(best_cross_vectors);
            phase_same_vectors[phase] = std::move(best_same_vectors);
            phase_count_pattern_vectors[phase] = std::move(best_count_pattern_vectors);
            phase_count_type_vectors[phase] = std::move(best_count_type_vectors);
            ...
            std::cout << "phase " << phase
                      ...
                      << " count_nonzero_vector_values " << result.count_nonzero_vector_values
                      << std::endl;
        }

        write_egev4(cfg, linear_values, phase_cross_vectors, phase_results, cfg.output_cross_egev4, 0);
        write_egev4(cfg, linear_values, phase_same_vectors, phase_results, cfg.output_same_egev4, 1);
        write_count_type_egct(cfg, phase_count_pattern_vectors, phase_count_type_vectors, phase_results);
        ...
        std::cout << "wrote_count_type " << cfg.output_count_type_egct << std::endl;

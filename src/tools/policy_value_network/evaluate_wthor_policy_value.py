#!/usr/bin/env python3
"""
Evaluate the policy-value network on WTHOR human-game board data.

Related issue: #613

The source board-data directory is currently named records1 in the training-data
tree, but generated evaluation artifacts use WTHOR in their file names.
"""
...
class BinaryPolicyValueNetwork:
    def __init__(self, path: Path):
        self.trunk_layers = []
        with path.open("rb") as f:
            magic = f.read(16)
            if magic != b"EGR_POLVAL_V1\0\0\0":
                raise ValueError(f"invalid policy-value weights magic in {path}")
            version, n_trunk_layers, input_size, policy_size, value_size = struct.unpack("<IIIII", f.read(20))
            if version != 1 or input_size != INPUT_SIZE or policy_size != POLICY_SIZE or value_size != 1 or n_trunk_layers <= 0:
                raise ValueError(f"unsupported policy-value weights header in {path}")
            ...
            self.policy_head, _ = self._read_layer(f, expected_input)
            self.value_head, _ = self._read_layer(f, expected_input)
    ...
    def predict(self, x: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        policy = softmax(self._forward_layer(y, self.policy_head))
        value = self._forward_layer(y, self.value_head).reshape(-1)
        return policy, value
...
def default_model_file() -> Path:
    return Path(__file__).resolve().parent / "trained" / "final_issue613_pv128w01" / "best_model.h5"


def default_weights_file() -> Path:
    return Path(__file__).resolve().parent / "trained" / "final_issue613_pv128w01" / "best_policy_value_network_weights.bin"
...
def records_to_features(records: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    values = records["score"].astype(np.float32, copy=False) / VALUE_SCALE
    ...
    return x, policies, values, legal, n_discs
...
def predict_batch(model, binary_network: Optional[BinaryPolicyValueNetwork], x: np.ndarray, batch_size: int) -> Tuple[np.ndarray, np.ndarray]:
    if model is not None:
        ...
        if isinstance(pred, dict):
            return np.asarray(pred["policy"], dtype=np.float32), np.asarray(pred["value"], dtype=np.float32).reshape(-1)
        if isinstance(pred, (list, tuple)):
            names = list(getattr(model, "output_names", []))
            if "policy" in names and "value" in names:
                by_name = dict(zip(names, pred))
                return np.asarray(by_name["policy"], dtype=np.float32), np.asarray(by_name["value"], dtype=np.float32).reshape(-1)
            if len(pred) == 2:
                return np.asarray(pred[0], dtype=np.float32), np.asarray(pred[1], dtype=np.float32).reshape(-1)
        raise RuntimeError("unsupported Keras prediction output")
...
def update_value_totals(totals: dict, predicted: np.ndarray, target: np.ndarray) -> None:
    error = predicted.astype(np.float64) - target.astype(np.float64)
    totals["positions"] += int(len(error))
    totals["abs_error_sum"] += float(np.sum(np.abs(error)))
    totals["squared_error_sum"] += float(np.sum(error * error))
    totals["error_sum"] += float(np.sum(error))
    sign_mask = target != 0.0
    if np.any(sign_mask):
        totals["sign_positions"] += int(np.count_nonzero(sign_mask))
        totals["sign_hits"] += int(np.count_nonzero(np.sign(predicted[sign_mask]) == np.sign(target[sign_mask])))


def value_row(name_key: str, name: str, totals: dict) -> dict:
    positions = totals["positions"]
    mae = totals["abs_error_sum"] / positions if positions else 0.0
    rmse = math.sqrt(totals["squared_error_sum"] / positions) if positions else 0.0
    mean_error = totals["error_sum"] / positions if positions else 0.0
    sign_positions = totals["sign_positions"]
    sign_accuracy = totals["sign_hits"] / sign_positions if sign_positions else 0.0
    return {
        name_key: name,
        "positions": positions,
        "mae": mae,
        ...
        "disc_mae": mae * VALUE_SCALE,
        "disc_rmse": rmse * VALUE_SCALE,
        "disc_mean_error": mean_error * VALUE_SCALE,
        "sign_hits": totals["sign_hits"],
...
def evaluate(args: argparse.Namespace) -> dict:
    ...
    if args.model is not None and args.model.exists():
        ...
        weights = args.weights if args.weights is not None else default_weights_file()
        binary_network = BinaryPolicyValueNetwork(weights)
    ...
    value_totals = empty_value_totals()
    value_by_phase = {phase: empty_value_totals() for phase in total_by_phase}
    ...
    for dat_file in dat_files:
        ...
        for records in iter_record_batches(dat_file, args.batch_size, remaining):
            ...
            x, policies, targets, legal, n_discs = records_to_features(records)
            probabilities, predicted_values = predict_batch(model, binary_network, x, args.predict_batch_size)
            ...
            update_value_totals(value_totals, predicted_values, targets)
            phase_names = phase_names_from_discs(n_discs)
            for phase in ("opening_4_20", "midgame_21_44", "endgame_45_64"):
                phase_mask = phase_names == phase
                if np.any(phase_mask):
                    update_value_totals(value_by_phase[phase], predicted_values[phase_mask], targets[phase_mask])
    ...
    value_summary = value_row("scope", "overall", value_totals)
    value_phase_rows = [value_row("phase", phase, totals) for phase, totals in value_by_phase.items()]
    result = {
        ...
        "value": value_summary,
        "value_by_phase": value_phase_rows,
    }

    args.output_dir.mkdir(parents=True, exist_ok=True)
    with (args.output_dir / "wthor_policy_value_evaluation.json").open("w") as f:
        json.dump(result, f, indent=2)
    write_csv(args.output_dir / "wthor_policy_value_topn_accuracy.csv", topn_rows, ["top_n", "hits", "positions", "accuracy"])
    write_csv(args.output_dir / "wthor_policy_value_topn_accuracy_by_phase.csv", phase_rows, ["phase", "top_n", "hits", "positions", "accuracy"])
    value_fields = ["scope", "positions", "mae", "rmse", "mean_error", "disc_mae", "disc_rmse", "disc_mean_error", "sign_hits", "sign_positions", "sign_accuracy"]
    phase_value_fields = ["phase", "positions", "mae", "rmse", "mean_error", "disc_mae", "disc_rmse", "disc_mean_error", "sign_hits", "sign_positions", "sign_accuracy"]
    write_csv(args.output_dir / "wthor_policy_value_metrics.csv", [value_summary], value_fields)
    write_csv(args.output_dir / "wthor_policy_value_metrics_by_phase.csv", value_phase_rows, phase_value_fields)
    return result
...
def make_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate policy-value network on WTHOR board data.")
    ...
    parser.add_argument("--output-dir", type=Path, default=Path(__file__).resolve().parent / "trained" / "wthor_policy_value_eval")
...
def main() -> None:
    ...
    value = result["value"]
    print(f"value mae {value['mae']:.6f}  rmse {value['rmse']:.6f}  disc_mae {value['disc_mae']:.3f}  sign {value['sign_accuracy'] * 100.0:.3f}%")

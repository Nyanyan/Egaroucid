...
Input features are fixed-color bitboards:
  [black exists on 64 squares, white exists on 64 squares]
...
def records_to_features(records: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    black_bits = np.where(black_to_move, player_bits, opponent_bits).astype(np.uint64, copy=False)
    white_bits = np.where(black_to_move, opponent_bits, player_bits).astype(np.uint64, copy=False)

    n = len(records)
    x = np.empty((n, INPUT_SIZE), dtype=np.float32)
    x[:, :64] = ((black_bits.reshape(-1, 1) & BIT_MASKS) != 0).astype(np.float32)
    x[:, 64:] = ((white_bits.reshape(-1, 1) & BIT_MASKS) != 0).astype(np.float32)

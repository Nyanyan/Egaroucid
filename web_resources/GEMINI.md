...
Gemini CLIを使った英訳・更新は歓迎します。ただし、作業を完全にGemini CLIへ任せず、最終的な内容は必ず人間が確認してください。

- Gemini CLIが生成・修正した文章は、すべての行を人間が確認してから反映してください。
- Gemini CLIを使った作業に関する責任は、そのGemini CLIを使った人間にあるものとします。
- Gemini CLIを使った変更をプルリクエストに含める場合は、Gemini CLIを使ったことをプルリクエスト本文で開示してください。
- プルリクエスト本文は人間が書くこと、またはGemini CLIを翻訳のみの用途に使うことを推奨します。
- Gemini CLIにプルリクエスト本文を書かせる場合は、使われている言葉が一般的に広く使われているかを確認し、理解しやすい文章にしてください。文章の意味が理解できない場合、追加で質問をすることや、断りなくプルリクエストをcloseすることがあります。
...
### 注意事項
Gemini CLIが完全に英訳した記事には、**必ず「This page was machine translated by Gemini CLI from the Japanese version. Please refer to the Japanese version for the official content.」**と記してください。
- この注意書きは太字で書いてください。
...
- **ファイルの存在確認**: `run_shell_command` で `ls` や `Test-Path` を使うと、ポリシーにより実行が拒否されることがあります。ファイルの存在を確認する際は、`glob` や `list_directory` ツールを優先して使用してください。

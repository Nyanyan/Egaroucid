...
- Current completed total: 20,125 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 20,125 position
samples:
...
- 現在の完了数: 20,125 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭20,125局面サンプルの top-1 symmetric accuracy:

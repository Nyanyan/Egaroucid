...
Parallel_task ybwc_do_task_nws(int id, uint64_t player, uint64_t opponent, int_fast8_t n_discs, uint_fast8_t parity, bool use_mpc, double mpct, int first_n_discs, int alpha, int depth, uint64_t legal, bool is_end_search, uint_fast8_t policy, const bool *searching){
    Search search;
    ...
    search.mpct = mpct;
    search.first_n_discs = first_n_discs;
    search.n_nodes = 0ULL;
...
inline bool ybwc_split_nws(const Search *search, const Flip *flip, int alpha, int depth, uint64_t legal, bool is_end_search, const bool *searching, uint_fast8_t policy, const int canput, const int pv_idx, const bool seems_to_be_all_node, const int split_count, vector<future<Parallel_task>> &parallel_tasks){
    if (thread_pool.n_idle() &&
        (pv_idx || seems_to_be_all_node)){
            bool pushed;
            parallel_tasks.emplace_back(thread_pool.push(&pushed, &ybwc_do_task_nws, search->board.player, search->board.opponent, search->n_discs, search->parity, search->use_mpc, search->mpct, search->first_n_discs, alpha, depth, legal, is_end_search, policy, searching));
            if (!pushed)
    ...
    Parallel_task ybwc_do_task_end(int id, uint64_t player, uint64_t opponent, int_fast8_t n_discs, uint_fast8_t parity, int first_n_discs, int alpha, int beta, uint64_t legal, uint_fast8_t policy, const bool *searching){
        Search search;
        search.board.player = player;
        search.board.opponent = opponent;
        search.n_discs = n_discs;
    ...
    inline bool ybwc_split_end(const Search *search, const Flip *flip, int alpha, int beta, uint64_t legal, const bool *searching, uint_fast8_t policy, const int canput, const int pv_idx, const bool seems_to_be_all_node, const int split_count, vector<future<Parallel_task>> &parallel_tasks){
        if (thread_pool.n_idle() &&
            ...
            search->n_discs <= HW2 - YBWC_END_SPLIT_MIN_DEPTH){
                bool pushed;
                parallel_tasks.emplace_back(thread_pool.push(&pushed, &ybwc_do_task_end, search->board.player, search->board.opponent, search->n_discs, search->parity, search->first_n_discs, alpha, beta, legal, policy, searching));
                if (!pushed)
                    parallel_tasks.pop_back();
                return pushed;
        }
    ...
    Parallel_task ybwc_do_task_end_nws(int id, uint64_t player, uint64_t opponent, int_fast8_t n_discs, uint_fast8_t parity, int first_n_discs, int alpha, uint64_t legal, uint_fast8_t policy, const bool *searching){
        Search search;
        search.board.player = player;
        search.board.opponent = opponent;
        search.n_discs = n_discs;
    ...
    inline bool ybwc_split_end_nws(const Search *search, const Flip *flip, int alpha, uint64_t legal, const bool *searching, uint_fast8_t policy, const int canput, const int pv_idx, const bool seems_to_be_all_node, const int split_count, vector<future<Parallel_task>> &parallel_tasks){
        if (thread_pool.n_idle() &&
            ...
            search->n_discs <= HW2 - YBWC_END_SPLIT_MIN_DEPTH){
                bool pushed;
                parallel_tasks.emplace_back(thread_pool.push(&pushed, &ybwc_do_task_end_nws, search->board.player, search->board.opponent, search->n_discs, search->parity, search->first_n_discs, alpha, legal, policy, searching));
                if (!pushed)
                    parallel_tasks.pop_back();
                return pushed;
        }

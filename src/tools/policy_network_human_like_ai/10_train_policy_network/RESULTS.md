...
- Current completed total: 10,665 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 10,665 position
samples:
...
- 現在の完了数: 10,665 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭10,665局面サンプルの top-1 symmetric accuracy:

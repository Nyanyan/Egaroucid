...
void print_usage(const char *exe) {
    std::cerr
        << "usage:\n"
        << "  " << exe << " weights.bin --board BOARD65 [--top N]\n"
        << "  " << exe << " weights.bin --transcript MOVES [--top N]\n"
        << "\n"
        << "BOARD65 is 64 board chars plus side-to-move char. X/0/* = black, O/1 = white, -/. = empty.\n"
...
int main(int argc, char **argv) {
    ...
    const auto prob = network.predict(board.black, board.white);

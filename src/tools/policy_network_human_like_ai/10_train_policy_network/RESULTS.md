...
- Current completed total: 11,565 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 11,565 position
samples:
...
- 現在の完了数: 11,565 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭11,565局面サンプルの top-1 symmetric accuracy:

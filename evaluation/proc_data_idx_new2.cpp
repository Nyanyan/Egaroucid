...
constexpr uint64_t canput_pattern_mask[N_CANPUT_PATTERNS] = {
...
constexpr int canput_pattern_n_cells[N_CANPUT_PATTERNS] = {
    6, 6, 6, 6, 
    6, 6, 6, 6
};
...
inline void calc_idx(int phase_idx, Board *b, int idxes[]){
    ...
    for (i = 0; i < N_SYMMETRY_PATTERNS; ++i){
        ...
        if (margin[i] != -1){
            idxes[i] *= 4;
            idxes[i] |= (1 & (player_mobility >> margin[i])) << 1;
            idxes[i] |= (1 & (opponent_mobility >> margin[i]));
        }
        */
    }
    ...
    for (int j = 0; j < N_CANPUT_PATTERNS; ++j)
        idxes[i++] = create_canput_pattern(player_mobility, opponent_mobility, j);
    /*

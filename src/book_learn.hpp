...
int book_learn_search(Board board, int level, const int book_depth, const int expected_value, const int best_value, const int expected_error, Board *board_copy, uint64_t *strt_tim, string book_file, string book_bak){
    ...
    if (legal == 0ULL){
        board.pass();
            g = -book_learn_search(board, level, book_depth, -expected_value, -best_value, expected_error, board_copy, strt_tim, book_file, book_bak);
        board.pass();
        return g;
    }
    Search_result best_move = ai(board, level, true, 0);
    if (best_move.value < expected_value - expected_error - 2)
        return SCORE_UNDEFINED;
    ...
    board.move(&flip);
        ...
        g = -book_learn_search(board, level, book_depth, -expected_value, SCORE_UNDEFINED, expected_error, board_copy, strt_tim, book_file, book_bak);
    ...
    for (uint_fast8_t cell = first_bit(&legal); legal; cell = next_bit(&legal)){
        ...
        board.move(&flip);
            ...
            if (-SCORE_MAX <= g && g <= SCORE_MAX && g >= expected_value - expected_error - 4){
                g = -book_learn_search(board, level, book_depth, -expected_value, -best_move.value, expected_error, board_copy, strt_tim, book_file, book_bak);
                if (-SCORE_MAX <= g && g <= SCORE_MAX){
...
inline void learn_book(Board root_board, int level, const int book_depth, const int expected_error, Board *board_copy, string book_file, string book_bak, bool *book_learning){
    ...
    int g = book_learn_search(root_board, level, book_depth, expected_score, SCORE_UNDEFINED, expected_error, board_copy, &strt_tim, book_file, book_bak);

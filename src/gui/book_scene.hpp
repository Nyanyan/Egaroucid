...
public:
    ...
    void update() override {
        ...
        //}
        Scene::SetBackground(getData().colors.green);
        getData().fonts.font(language.get("book", "book_recalculate_leaf")).draw(25, 50, 50, getData().colors.white);
        if (before_start){
            start_button.draw();
            if (start_button.clicked()){
                before_start = false;
                task_future = std::async(std::launch::async, book_recalculate_leaf, getData().menu_elements.level, &stop);
            }
            back_button.draw();
            if (back_button.clicked() || KeyEscape.pressed()){
                getData().graph_resources.need_init = false;

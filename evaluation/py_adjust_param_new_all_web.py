...
#task_phases = [1, 0]
#task_phases = list(reversed(range(26, 56)))
task_phases = list(reversed(range(10)))
#task_phases = list(set(range(60)) - set([38]))
#task_phases.sort(reverse=True)
...
def do_task(i, i_e):
    ...
    for j in range(i, i_e):
        tasks.append(subprocess.Popen(('python py_adjust_param_new_web.py ' + str(task_phases[j])).split(), stderr=None, stdout=subprocess.PIPE))
    for j in range(i, i_e):

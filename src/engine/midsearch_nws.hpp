...
/*
    ...
    @return the value
*/
int nega_alpha_ordering_nws_simple(Search *search, int alpha, const int depth, const bool skipped, uint64_t legal, const bool *searching) {
...
int nega_alpha_ordering_nws(Search *search, int alpha, const int depth, const bool skipped, uint64_t legal, const bool is_end_search, const bool *searching, const bool *searching2, const bool *searching3) {
    if (!global_searching || !(*searching) || !(*searching2) || !(*searching3)) {
        return SCORE_UNDEFINED;
    }
    if (is_end_search) {
        ...
        */
        ...
        if (depth <= MID_SIMPLE_DEPTH) {
            return nega_alpha_ordering_nws_simple(search, alpha, depth, skipped, legal, searching3);
        }
    }
    int v = -SCORE_INF;
    ++search->n_nodes;
    ...
    if (legal == 0ULL) {
        ...
        search->pass();
            v = -nega_alpha_ordering_nws(search, -alpha - 1, depth, true, LEGAL_UNDEFINED, is_end_search, searching, searching2, searching3);
        search->pass();
        return v;
    }
    uint32_t hash_code = search->board.hash();
    ...
    #if USE_MID_MPC
        if (mpc(search, alpha, alpha + 1, depth, legal, is_end_search, &v, searching, searching2, searching3)) {
            return v;
        }
    #endif
    int best_move = TRANSPOSITION_TABLE_UNDEFINED;
    ...
    #endif
    move_list_evaluate_nws(search, move_list, moves, depth, alpha, searching3);
    #if USE_YBWC_NWS
        if (
            ...
            (searching == searching2 || searching2 == searching3)
        ) {
            move_list_sort(move_list);
            if (move_list[0].flip.flip) {
                search->move(&move_list[0].flip);
                    g = -nega_alpha_ordering_nws(search, -alpha - 1, depth - 1, false, move_list[0].n_legal, is_end_search, searching, searching2, searching3);
                ...
                if (v <= alpha) {
                    ybwc_search_young_brothers_nws(search, alpha, &v, &best_move, canput - 1, hash_code, depth, is_end_search, move_list, searching, searching2);
                }
            }
        } else{
    #endif
            for (int move_idx = 0; move_idx < canput - etc_done_idx && *searching && *searching2 && *searching3; ++move_idx) {
                swap_next_best_move(move_list, move_idx, canput);
                #if USE_MID_ETC
                    if (move_list[move_idx].flip.flip == 0) {
                        break;
                ...
                #endif
                search->move(&move_list[move_idx].flip);
                    g = -nega_alpha_ordering_nws(search, -alpha - 1, depth - 1, false, move_list[move_idx].n_legal, is_end_search, searching, searching2, searching3);
                search->undo(&move_list[move_idx].flip);
                if (v < g) {
                    v = g;
                    best_move = move_list[move_idx].flip.pos;

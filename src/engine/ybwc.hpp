...
*/
Parallel_task ybwc_do_task_nws(uint64_t player, uint64_t opponent, int_fast8_t n_discs, uint_fast8_t parity, uint_fast8_t mpc_level, bool is_presearch, int alpha, int beta, int depth, uint64_t legal, bool is_end_search, uint_fast8_t policy, int move_idx, const bool *searching){
    Search search;
    ...
    task.value = -nega_scout(&search, alpha, beta, depth, false, legal, is_end_search, searching);
    if (!(*searching))
...
*/
inline int ybwc_split_nws(Search *search, int alpha, int beta, int depth, uint64_t legal, bool is_end_search, const bool *searching, uint_fast8_t policy, const int move_idx, const int canput, const int running_count, std::vector<std::future<Parallel_task>> &parallel_tasks){
    if (
        ...
        if (transposition_cutoff_nws(search, search->board.hash(), depth, alpha, &v)){
            ...
            bool pushed;
            parallel_tasks.emplace_back(thread_pool.push(&pushed, std::bind(&ybwc_do_task_nws, search->board.player, search->board.opponent, search->n_discs, search->parity, search->mpc_level, search->is_presearch, alpha, beta, depth, legal, is_end_search, policy, move_idx, searching)));
            if (pushed)
                return YBWC_PUSHED;
            else
            ...
            if (move_list[move_idx].flip.flip){
                ...
                */
                ...
                search->move(&move_list[move_idx].flip);
                    int ybwc_split_state = ybwc_split_nws(search, -alpha - 1, -alpha, depth - 1, move_list[move_idx].n_legal, is_end_search, &n_searching, move_list[move_idx].flip.pos, move_idx, canput, running_count, parallel_tasks);
    ...
    void ybwc_search_young_brothers(Search *search, int *alpha, int *beta, int *v, int *best_move, uint32_t hash_code, int depth, bool is_end_search, std::vector<Flip_value> &move_list, bool need_best_move, const bool *searching){
        ...
        for (int move_idx = 1; move_idx < canput && n_searching; ++move_idx){
            ...
            if (move_list[move_idx].flip.flip){
                ...
                bool move_done = false, serial_searched = false;
                search->move(&move_list[move_idx].flip);
                    int ybwc_split_state = ybwc_split_nws(search, -(*beta), -(*alpha), depth - 1, move_list[move_idx].n_legal, is_end_search, &n_searching, move_list[move_idx].flip.pos, move_idx, canput, running_count, parallel_tasks);
                    if (ybwc_split_state == YBWC_PUSHED){
                        ++running_count;
                    } else{
                        if (ybwc_split_state == YBWC_NOT_PUSHED){
        ...
        /*
        if (*searching && fail_high_idx >= 0){
            if (*alpha < *beta){
                search->move(&move_list[fail_high_idx].flip);
                    g = -nega_scout(search, -(*beta), -(*alpha), depth - 1, false, move_list[fail_high_idx].n_legal, is_end_search, searching);
                search->undo(&move_list[fail_high_idx].flip);
                if (g < *v){
                    std::cerr << "ERR " << g << " " << *v << std::endl;
                }
                *alpha = g;
                *v = g;
                *best_move = move_list[fail_high_idx].flip.pos;

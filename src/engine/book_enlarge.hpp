...
int book_widen_search(Board board, int level, const int book_depth, int expected_error, double max_sum_error, Board *board_copy, int *player, uint64_t *strt_tim, std::string book_file, std::string book_bak, uint64_t strt){
    ...
    if (get_level_complete_depth(level) >= HW2 - board.n_discs() || max_sum_error <= 0.0)
        expected_error = 0;
    ...
    Flip flip;
    std::vector<int> book_best_moves = book.get_all_best_moves(&board);
    for (int policy: book_best_moves){
        ...
        *player ^= 1;
            ...
            if (global_searching && g >= -HW2 && g <= HW2){
                v = g;
                std::cerr << "time " << ms_to_time_short(tim() - strt) << " depth " << board.n_discs() - 4 << " BP value " << g << " best " << v << " remaining error " << max_sum_error << std::endl;
            } else
                return SCORE_UNDEFINED;
            ...
            *player ^= 1;
            board.copy(board_copy);
                ...
                if (global_searching && g >= alpha){
                    ...
                    if (global_searching && g >= -HW2 && g <= HW2){
                        v = std::max(v, g);
                        std::cerr << "time " << ms_to_time_short(tim() - strt) << " depth " << board.n_discs() - 4 << " AD value " << g << " best " << v << " remaining error " << max_sum_error - pre_error << std::endl;
                    }
                }
            board.undo_board(&flip);
            board.copy(board_copy);
...
inline void book_widen(Board root_board, int level, const int book_depth, int expected_error, Board *board_copy, int *player, std::string book_file, std::string book_bak, bool *book_learning){
    ...
    double max_sum_error = calc_book_widen_sum_error(root_board, book_depth, expected_error);

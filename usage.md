...
```人間的ヒント```にチェックすることで表示でき、各マスの右下におすすめ順位として表示されます。1と表示されるマスが一番おすすめです。

#### うみがめ数表示

```うみがめ数```にチェックすることでうみがめ数を表示できます。
...
学習を開始すると```book学習```ボタンが```学習停止```ボタンに変わります。```学習停止```ボタンを押すといつでも学習を停止できます。なお、学習中はアプリを閉じられません。アプリを閉じる前に必ず```学習停止```ボタンを押してください。

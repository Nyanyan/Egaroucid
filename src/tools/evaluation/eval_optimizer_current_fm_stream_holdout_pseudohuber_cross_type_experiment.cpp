/*
    ...
    Current-linear fixed cross-type FM optimizer with streaming chunk
    ...
    feature, including the stone-count feature.  The FM second-order
    interaction and vector updates use only cross-pattern-type pairs among the
    64 board-pattern features.  Interactions between the four orientations of
    the same pattern type and interactions with the stone-count feature are
    excluded from the FM term.
*/
...
Config parse_args(int argc, char **argv) {
    ...
    if (cfg.output_egev4.empty()) {
        usage_error("--output-egev4 is required");
    }
...
double predict_record(
    ...
    for (int feature_idx = 0; feature_idx < N_FEATURES; ++feature_idx) {
        ...
        for (int d = 0; d < dim; ++d) {
            ...
            sums[d] += vec;
            sums_sq[d] += vec * vec;
            group_sums[group_base + d] += vec;
            group_sums_sq[group_base + d] += vec * vec;
        }
    }
    ...
    for (int d = 0; d < dim; ++d) {
        ...
        for (int pattern_type = 0; pattern_type < N_PATTERN_TYPES; ++pattern_type) {
            ...
            same_type_interaction += group_sums[group_idx] * group_sums[group_idx] - group_sums_sq[group_idx];
        }
        interaction += sums[d] * sums[d] - sums_sq[d] - same_type_interaction;
    }
...
Metrics compute_metrics(
    ...
    std::vector<double> sums_sq(static_cast<size_t>(dim), 0.0);
    std::vector<double> group_sums(static_cast<size_t>(N_PATTERN_TYPES * dim), 0.0);
    std::vector<double> group_sums_sq(static_cast<size_t>(N_PATTERN_TYPES * dim), 0.0);
    for (int file_id : file_ids) {
        const fs::path path = data_path(root, phase, file_id);
        std::ifstream in(path, std::ios::binary);
        if (!in) {
        ...
        while (max_records_per_file <= 0 || n_file < static_cast<uint64_t>(max_records_per_file)) {
            ...
            const double pred = predict_record(linear, vectors, record, dim, sums, sums_sq, group_sums, group_sums_sq);
...
void update_record(
    ...
    const double pred = predict_record(linear, vectors, record, cfg.dim, sums, sums_sq, group_sums, group_sums_sq);
    ...
    for (int feature_idx = 0; feature_idx < N_PATTERN_FEATURES; ++feature_idx) {
        ...
        for (int d = 0; d < cfg.dim; ++d) {
            const double vec = vectors[base + d];
            const double grad = -residual * (sums[d] - group_sums[group_base + d]) + cfg.l2 * vec;
...
uint64_t train_one_epoch(
    ...
    std::vector<double> sums(static_cast<size_t>(cfg.dim), 0.0);
    std::vector<double> sums_sq(static_cast<size_t>(cfg.dim), 0.0);
    std::vector<double> group_sums(static_cast<size_t>(N_PATTERN_TYPES * cfg.dim), 0.0);
    std::vector<double> group_sums_sq(static_cast<size_t>(N_PATTERN_TYPES * cfg.dim), 0.0);
    ...
    for (int file_id : file_order) {
        ...
        while (cfg.max_records_per_file <= 0 || n_file < static_cast<uint64_t>(cfg.max_records_per_file)) {
            ...
            for (const Record &record : chunk) {
                update_record(record, phase, cfg, linear, vectors, m, v, initialized, step, beta1_power, beta2_power, sums, sums_sq, group_sums, group_sums_sq);
                ++updates;
            }
            ++chunk_idx;
        }
...
PhaseResult train_phase(
    ...
    result.nonzero_vector_values = 0;
    if (result.vector_scale <= 0.0) {
        result.vector_scale = 1.0;
    }
    for (int param_id = 0; param_id < N_PARAMS_PER_PHASE; ++param_id) {
...
void write_egev4(
    ...
    fs::path output_path(cfg.output_egev4);
    ...
    for (int phase = 0; phase < N_PHASES; ++phase) {
        ...
        write_f32(out, it == phase_results.end() ? 1.0f : static_cast<float>(it->second.vector_scale));
    }
...
void write_summary(
    ...
    out << "optimizer=current-linear-fixed cross-type-FM streaming Adam\n";
...
int main(int argc, char **argv) {
    try {
        ...
        for (int phase : cfg.phases) {
            ...
            PhaseResult result = train_phase(cfg, data_root, phase, linear_values, best_vectors);
        ...
        write_egev4(cfg, linear_values, phase_vectors, phase_results);

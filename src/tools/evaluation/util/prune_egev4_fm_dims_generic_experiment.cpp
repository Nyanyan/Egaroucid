/*
    ...
    @file prune_egev4_fm_dims_generic_experiment.cpp
        Prune quantized EGEV4 FM latent dimensions by per-phase vector energy.
        This generic experiment variant accepts the params-per-phase value from
        the EGEV4 header, so it can be used for current-model and 7.7beta files.
    @date 2026
...
[[noreturn]] void usage_error(const std::string &message) {
    throw std::runtime_error(message + "\n"
        "usage: prune_egev4_fm_dims_generic_experiment --input FILE --output FILE "
        "--output-dim 8 [--summary FILE] [--timestamp now|YYYYMMDDhhmmss]");
}
...
std::vector<int> select_top_energy_dims(
    ...
    for (int param = 0; param < info.params_per_phase; ++param) {
...
std::vector<uint8_t> prune_egev4(
    ...
    const size_t output_phase_stride = static_cast<size_t>(info.params_per_phase) * output_record_stride;
...
void write_summary(const Config &cfg, const Egev4Info &info, const std::vector<PhaseSummary> &summaries) {
    ...
    out << "Generic EGEV4 FM dimension prune experiment\n";
    out << "input=" << cfg.input << "\n";
    out << "output=" << cfg.output << "\n";
    out << "input_timestamp=" << info.timestamp << "\n";
    out << "version=" << info.version << "\n";
    out << "n_phases=" << info.n_phases << "\n";
    out << "params_per_phase=" << info.params_per_phase << "\n";
...
int main(int argc, char **argv) {
    try {
        ...
        std::cout << "params_per_phase " << info.params_per_phase << std::endl;

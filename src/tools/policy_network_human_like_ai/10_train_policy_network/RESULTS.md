...
- Current completed total: 16,865 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 16,865 position
samples:
...
- 現在の完了数: 16,865 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭16,865局面サンプルの top-1 symmetric accuracy:

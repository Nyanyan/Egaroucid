...
- Current completed total: 19,225 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 19,225 position
samples:
...
- 現在の完了数: 19,225 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭19,225局面サンプルの top-1 symmetric accuracy:

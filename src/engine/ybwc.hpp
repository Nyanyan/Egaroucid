...
*/
Parallel_task ybwc_do_task_nws(uint64_t player, uint64_t opponent, int_fast8_t n_discs, uint_fast8_t parity, uint_fast8_t mpc_level, bool is_presearch, int parent_alpha, const int depth, uint64_t legal, const bool is_end_search, uint_fast8_t policy, int move_idx, const bool *searching) {
...
*/
inline int ybwc_split_nws(Search *search, int parent_alpha, const int depth, uint64_t legal, const bool is_end_search, const bool *searching, uint_fast8_t policy, const int move_idx, const int canput, const int running_count, std::vector<std::future<Parallel_task>> &parallel_tasks) {
    if (
    ...
    inline void ybwc_search_young_brothers_nws(Search *search, int alpha, int *v, int *best_move, uint32_t hash_code, int depth, bool is_end_search, std::vector<Flip_value> &move_list, const bool *searching) {
        ...
        std::unique_ptr<bool> n_searching(new bool(true));
        //bool n_searching = true;
        int canput = (int)move_list.size();
        int running_count = 0;
        int g;
        for (int move_idx = 1; move_idx < canput && *n_searching.get()  && *searching; ++move_idx) {
            if (move_list[move_idx].flip.flip) {
                bool serial_searched = false;
                search->move(&move_list[move_idx].flip);
                    int ybwc_split_state = ybwc_split_nws(search, alpha, depth - 1, move_list[move_idx].n_legal, is_end_search, n_searching.get(), move_list[move_idx].flip.pos, move_idx, canput, running_count, parallel_tasks);
                    if (ybwc_split_state == YBWC_PUSHED) {
                        ++running_count;
                    } else {
                        if (ybwc_split_state == YBWC_NOT_PUSHED) {
                            *n_searching.get() &= *searching;
...
/*
    ...
    inline void ybwc_search_young_brothers_nws(Search *search, int alpha, int *v, int *best_move, const uint32_t hash_code, const int depth, const bool is_end_search, std::vector<Flip_value> &move_list, const bool *searching) {
        ...
        for (int move_idx = 1; move_idx < canput && *n_searching; ++move_idx) {
            if (move_list[move_idx].flip.flip) {
                search->move(&move_list[move_idx].flip);
                    int ybwc_split_state = ybwc_split_nws(search, -alpha - 1, depth - 1, move_list[move_idx].n_legal, is_end_search, n_searching, move_list[move_idx].flip.pos, move_idx, canput, running_count, parallel_tasks, &not_searching);
                    if (ybwc_split_state == YBWC_PUSHED) {
                        ++running_count;
                    } else {
                        if (ybwc_split_state == YBWC_NOT_PUSHED) {

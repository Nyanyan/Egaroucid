...
- Current completed total: 18,445 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 18,445 position
samples:
...
- 現在の完了数: 18,445 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭18,445局面サンプルの top-1 symmetric accuracy:

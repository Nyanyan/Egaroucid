/*
    ...
    @file eval_optimizer_cuda_robust_adam_anchor.cu
        Evaluation Function Optimizer in CUDA, robust Adam with anchor regularization experiment
    @date 2021-2024
...
#define USE_ROBUST_RESIDUAL_CLIP true
#define ROBUST_RESIDUAL_CLIP_DISC_DIFF 24.0
...
__global__ void adj_calculate_residual(const double *device_eval_arr, const int n_data, const int *device_start_idx_arr, const Adj_Data *device_train_data, int *device_rev_idx_arr, double *device_residual_arr, double *device_error_monitor_arr){
    ...
    double update_error = residual_error;
#if USE_ROBUST_RESIDUAL_CLIP
    const double max_update_error = ROBUST_RESIDUAL_CLIP_DISC_DIFF * ADJ_STEP;
    if (update_error > max_update_error) {
        update_error = max_update_error;
    } else if (update_error < -max_update_error) {
        update_error = -max_update_error;
    }
#endif
    for (int i = 0; i < ADJ_N_FEATURES; ++i){
        #if ADJ_CELL_WEIGHT
            if (device_train_data[data_idx].features[i] < 10){
                atomicAdd(&device_residual_arr[device_train_data[data_idx].features[i]], update_error);
                atomicAdd(&device_residual_arr[device_rev_idx_arr[device_train_data[data_idx].features[i]]], update_error);
            } else if (device_train_data[data_idx].features[i] < 20){
                atomicAdd(&device_residual_arr[device_train_data[data_idx].features[i] - 10], -update_error);
                atomicAdd(&device_residual_arr[device_rev_idx_arr[device_train_data[data_idx].features[i] - 10]], -update_error);
            }
        #else
            atomicAdd(&device_residual_arr[device_start_idx_arr[i] + (int)device_train_data[data_idx].features[i]], update_error);
            ...
            atomicAdd(&device_residual_arr[rev_idx], update_error);
        #endif
    }
...
*/
__global__ void adam(const int phase, const int eval_size, double *device_eval_arr, const double *device_anchor_eval_arr, int *device_n_appear_arr, double *device_residual_arr, double alpha_stab, double *device_m_arr, double *device_v_arr, const int n_loop, const double anchor_lambda, const double anchor_max_delta){
    const int eval_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (eval_idx >= eval_size){
        return;
    }
    if (device_n_appear_arr[eval_idx] > ADJ_IGNORE_N_APPEAR || (phase <= 11 && device_n_appear_arr[eval_idx] > 0)) {
        ...
        if (anchor_lambda > 0.0){
            double delta = device_eval_arr[eval_idx] - device_anchor_eval_arr[eval_idx];
            grad -= 2.0 * anchor_lambda * (double)device_n_appear_arr[eval_idx] * delta;
        }
        constexpr double beta1 = 0.9;
        constexpr double beta2 = 0.999;
        constexpr double epsilon = 1e-7;
        ...
        if (anchor_max_delta > 0.0){
            double upper = device_anchor_eval_arr[eval_idx] + anchor_max_delta;
            double lower = device_anchor_eval_arr[eval_idx] - anchor_max_delta;
            if (device_eval_arr[eval_idx] > upper) {
                device_eval_arr[eval_idx] = upper;
            }
            if (device_eval_arr[eval_idx] < lower) {
                device_eval_arr[eval_idx] = lower;
            }
        }
#if USE_PARAM_LIMIT
        if (device_eval_arr[eval_idx] > ADJ_EVAL_PARAM_MAX) {
...
int main(int argc, char* argv[]) {
    ...
    double *host_anchor_eval_arr = (double*)malloc(sizeof(double) * eval_size); // initial eval array for anchor regularization
    ...
    if (host_eval_arr == nullptr || host_anchor_eval_arr == nullptr || host_rev_idx_arr == nullptr || host_train_data == nullptr || host_val_error_monitor_arr == nullptr){
    ...
    std::copy(host_eval_arr, host_eval_arr + eval_size, host_anchor_eval_arr);
    ...
    // shuffle data
    constexpr uint32_t SHUFFLE_SEED_BASE = 20260629U;
    std::mt19937 engine(SHUFFLE_SEED_BASE + (uint32_t)phase);
    ...
    double *device_anchor_eval_arr; // device initial eval array for anchor regularization
    ...
    cudaMalloc(&device_anchor_eval_arr, sizeof(double) * eval_size);
    ...
    cudaMemcpy(device_anchor_eval_arr, host_anchor_eval_arr, sizeof(double) * eval_size, cudaMemcpyHostToDevice);
    ...
    double anchor_lambda = get_env_double("EGAROUCID_EVAL_ANCHOR_LAMBDA", 0.01);
    double anchor_max_delta = get_env_double("EGAROUCID_EVAL_ANCHOR_MAX_DELTA", 0.0);
    std::cerr << "anchor_lambda " << anchor_lambda << " anchor_max_delta " << anchor_max_delta << std::endl;
    cudaMemset(device_residual_arr, 0, sizeof(double) * eval_size);
    ...
    while (tim() - strt < msecond) {
        ...
        adam <<<n_blocks_next_step, N_THREADS_PER_BLOCK_NEXT_STEP>>> (phase, eval_size, device_eval_arr, device_anchor_eval_arr, device_n_appear_arr, device_residual_arr, alpha_stab, device_m_arr, device_v_arr, n_loop, anchor_lambda, anchor_max_delta);
#if USE_WARMUP
        if (alpha_stab < alpha) {
    ...
    std::cerr << "phase " << phase << " time " << (tim() - strt) << " ms n_train_data " << n_train_data << " n_val_data " << n_val_data << " score_avg " << score_avg << " n_loop " << n_loop << " MSE " << host_error_monitor_arr[0] << " MAE " << host_error_monitor_arr[1] << " val_MSE " << host_val_error_monitor_arr[0] << " val_MAE " << host_val_error_monitor_arr[1] << " (with int) alpha " << alpha_in << " anchor_lambda " << anchor_lambda << " anchor_max_delta " << anchor_max_delta << " n_patience " << n_patience << " reduce_lr_patience " << reduce_lr_patience << " reduce_lr_ratio " << reduce_lr_ratio << std::endl;

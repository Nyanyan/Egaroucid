...
constexpr int AI_TL_GGS_DEFENSIVE_ALT_VERIFY_EARLY_MAX_N_EMPTY = 50;
constexpr int AI_TL_GGS_DEFENSIVE_ALT_VERIFY_EARLY_MIN_DEPTH = 28;
constexpr int AI_TL_GGS_DEFENSIVE_ALT_VERIFY_MIN_DEPTH = 32;
constexpr int AI_TL_GGS_DEFENSIVE_ALT_VERIFY_MAX_N_EMPTY = 50;
...
constexpr double AI_TL_GGS_AMBIGUITY_PROBE_TIME_COE = 0.18;
...
constexpr uint64_t AI_TL_GGS_AMBIGUITY_BONUS_PER_CLOSE_MOVE = 4200ULL;
constexpr uint64_t AI_TL_GGS_AMBIGUITY_BONUS_PER_SHALLOW_CLOSE_MOVE = 1600ULL;
constexpr uint64_t AI_TL_GGS_AMBIGUITY_TINY_GAP_BONUS = 3500ULL;
constexpr uint64_t AI_TL_GGS_AMBIGUITY_NARROW_GAP_BONUS = 5500ULL;
constexpr uint64_t AI_TL_GGS_AMBIGUITY_MAX_BONUS = 36000ULL;
constexpr double AI_TL_GGS_AMBIGUITY_MAX_BONUS_COE = 2.20;
...
constexpr uint64_t AI_TL_GGS_AMBIGUITY_UNRELIABLE_BASE_BONUS = 3000ULL;
constexpr uint64_t AI_TL_GGS_AMBIGUITY_UNRELIABLE_BONUS_PER_CLOSE_MOVE = 2600ULL;
constexpr uint64_t AI_TL_GGS_AMBIGUITY_UNRELIABLE_TINY_GAP_BONUS = 6500ULL;
...
inline uint64_t ai_time_limit_ggs_ambiguity_boost(const Board &board, const std::vector<Ponder_elem> &move_list, uint64_t time_limit, uint64_t remaining_time_msec, bool show_log) {
    ...
    if (reliable_close_moves < 2) {
        ...
        const double second_gap = best_value - second_value;
        if (
            -AI_TL_GGS_AMBIGUITY_UNRELIABLE_TINY_GAP_BEST_ABS_VALUE <= best_value &&
            best_value <= AI_TL_GGS_AMBIGUITY_UNRELIABLE_TINY_GAP_BEST_ABS_VALUE &&
...
void ai_get_value_until(Board board, Ponder_elem *elem, uint64_t deadline, thread_id_t thread_id) {
    while (tim() < deadline && !elem->is_complete_search && global_searching) {
        Board n_board = board.copy();
        n_board.move_board(&elem->flip);
        const int max_depth = HW2 - n_board.n_discs();
        ...
        const bool new_is_end_search = new_depth == max_depth;
        const bool new_is_complete_search = new_is_end_search && new_mpc_level == MPC_100_LEVEL;
        Search search(&n_board, new_mpc_level, true, false);
        search.thread_id = thread_id;
        alignas(std::atomic_ref<bool>::required_alignment) bool n_searching = true;
        const uint64_t time_limit_this_search = deadline > tim() ? deadline - tim() : 0ULL;
        std::future<int> v_future = std::async(
            std::launch::async,
            nega_scout,
            &search,
            -SCORE_MAX,
        ...
        if (v_future.wait_for(std::chrono::milliseconds(time_limit_this_search)) == std::future_status::ready) {
            const int v = -v_future.get();
            if (global_searching && search_cancellation_load(&n_searching)) {
...
void ai_get_value_after_start(
    ...
    while (!start->load(std::memory_order_acquire) && global_searching) {
        std::this_thread::yield();
    }
    if (global_searching) {
        ai_get_value_until(board, elem, *deadline, thread_id);
    }
}
...
std::vector<Ponder_elem> ai_get_values(Board board, bool show_log, uint64_t time_limit, thread_id_t thread_id) {
    ...
    const int n_usable_threads = max_helper_threads + 1;
    const bool use_candidate_parallel =
        USE_AI_GET_VALUES_CANDIDATE_PARALLEL &&
        canput >= 2 &&
        n_usable_threads >= 2 &&

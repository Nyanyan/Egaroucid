...
最終更新: 2024/12/24 Egaroucid 7.5.0
...
どの石がどちらのプレイヤにとってどれくらい有利かを可視化する機能です。この機能をオンにすると、一部のマスに色がつきます。橙色は黒有利、青色は白有利の石を表します。色は濃いほど有利な度合いが大きいことを表します。

例として以下の画像では、左上の方は白有利、下側および右側は黒有利が多いと表示されています。これは、左上は真っ黒で黒から手を付けられないため白有利、下側と右側は真っ白で白から手を付けられないため黒有利と判断されています。
...
Egaroucidでは、テキスト形式の入力として、棋譜、ボード情報、GGF (Generic Game Format)、オセロクエスト形式、の4種類をサポートしています。「クリップボードから形式を指定せず入力」は、Egaroucid側がこれらの形式を自動で判定する機能です。また、画面遷移なしにクリップボードから直接情報を読み取るため、非常にスピーディーに入力を行えます。デフォルトではCtrl+Vをこのショートカットキーとして割り当ててあります。

対応しているそれぞれのフォーマットについては以下を参照してください。
...
バージョン6.5.2からカーソル移動ができるようになりました。また、バージョン7.5.0からパスを表す文字"PA"や"PS"、"pa"、"ps"が入っていても読み込めるようになりました。
...
Generic Game Formatで記述されたゲームを入力できます。Generic Game Formatは以下のようなフォーマットです。

<code>(;GM[Othello]PC[]PB[b_player]PW[w_player]RE[-32]TI[0]TY[8]BO[8 ---------------------------O*------*O--------------------------- *]B[E6]W[F6]B[F5]W[D6]B[C3]W[G4]B[C6]W[F4]B[H4]W[F3]B[G5]W[E3]B[D3]W[C4]B[C5]W[G6]B[H6]W[H3]B[G3]W[E2]B[E1]W[D7]B[D8]W[B3]B[B5]W[F7]B[F8]W[D2]B[D1]W[C7]B[A3]W[H5]B[H2]W[C8]B[F2]W[E8]B[B8]W[E7]B[B4]W[A4]B[A6]W[A5]B[B6]W[H7]B[H8]W[G8]B[G7]W[A8]B[PA]W[A7]B[PA]W[B7]B[PA]W[A2]B[B2]W[G2]B[F1]W[H1]B[PA]W[A1]B[C2]W[G1]B[B1]W[C1];)</code>

本機能では、黒番と白番のプレイヤ名も自動で取得します。



### 入力 > オセロクエスト形式で入力

[オセロクエスト](http://questgames.net/reversi)は独自形式で棋譜を保存していますが、その形式をEgaroucidに読み込ませることができます。本機能では、黒番と白番のプレイヤ名も自動で取得します。

オセロクエスト形式は以下のようなフォーマットです。
...
盤面のスクリーンショットを撮れます。クリップボードに画像をコピーする他、ドキュメントフォルダ内のEgaroucid/screenshotsフォルダに日時+棋譜をファイル名として画像が保存されます。
...
「画像を保存」で、クリップボードに画像をコピーしつつ、画像を保存します。保存先はドキュメントフォルダ内のEgaroucid/screenshotsフォルダで、日時+棋譜をファイル名とします。

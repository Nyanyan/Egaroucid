...
inline int ybwc_split_nws(Search *search, int alpha, int depth, uint64_t legal, bool is_end_search, const bool *searching, uint_fast8_t policy, const int move_idx, const int canput, const int running_count, std::vector<std::future<Parallel_task>> &parallel_tasks){
    ...
    void ybwc_search_young_brothers(Search *search, int *alpha, int *beta, int *v, int *best_move, uint32_t hash_code, int depth, bool is_end_search, std::vector<Flip_value> &move_list, bool need_best_move, const bool *searching){
                ...
                search->move(&move_list[move_idx].flip);
                    ...
                    if (ybwc_split_state == YBWC_PUSHED){
                        ...
                        if (*searching){
                            ...
                            if (*alpha < g){
                                std::cerr << "update " << *alpha << " " << g << " " << *beta << std::endl;
        ...
        */
        if (*searching && research_idxes.size() && next_alpha < *beta){
            ...
            *alpha = next_alpha;
            for (const int &research_idx: research_idxes){
                ...
                std::cerr << *alpha << " " << next_alpha << " " << g << std::endl;
                if (*alpha < g){
                    *alpha = g;
                    if (*alpha >= *beta){
                        break;
            ...
            std::cerr << research_idxes.size() << " " << prev_alpha << " " << next_alpha << " " << *alpha << " , " << *beta << std::endl;
            if (*alpha < *beta){

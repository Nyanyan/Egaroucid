/*
    ...
    Current-linear trainable joint tri-type FM optimizer with streaming chunk
    ...
    current source files.  Unlike the fixed-linear tri-type experiment, this
    tool also updates the current-model linear weights.  The linear weights are
    initialized from input EGEV2 and regularized toward those initial values.
    Three independent FM vector banks are trained against the same residual:
...
struct Config {
    ...
    double linear_l2 = 0.00005;
    double linear_lr_scale = 0.1;
    double linear_delta_clip = 32.0;
...
struct PhaseResult {
    ...
    double linear_max_abs_delta = 0.0;
    uint64_t linear_changed_params = 0;
...
Config parse_args(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--linear-lr-scale") {
            cfg.linear_lr_scale = std::stod(require_value(i, argc, argv));
        } else if (arg == "--linear-delta-clip") {
            cfg.linear_delta_clip = std::stod(require_value(i, argc, argv));
    ...
    if (!std::isfinite(cfg.linear_l2) || cfg.linear_l2 < 0.0) {
        usage_error("--linear-l2 must be finite and non-negative");
    }
    if (!std::isfinite(cfg.linear_lr_scale) || cfg.linear_lr_scale < 0.0) {
        usage_error("--linear-lr-scale must be finite and non-negative");
    }
    if (!std::isfinite(cfg.linear_delta_clip) || cfg.linear_delta_clip < 0.0) {
        usage_error("--linear-delta-clip must be finite and non-negative");
    }
...
void update_record(
    ...
    int phase,
    const Config &cfg,
    const std::vector<float> &linear_base,
    ...
    if (cfg.linear_lr_scale > 0.0) {
        const double linear_lr_t = lr_t * cfg.linear_lr_scale;
        for (int feature_idx = 0; feature_idx < N_FEATURES; ++feature_idx) {
            const uint32_t param_id = record.active_ids[feature_idx];
            const double linear_value = linear[param_id];
            const double linear_delta = linear_value - linear_base[param_id];
            const double linear_grad = -residual + cfg.linear_l2 * linear_delta;
            const double next_linear_m = 0.9 * linear_m[param_id] + 0.1 * linear_grad;
            const double next_linear_v = 0.999 * linear_v[param_id] + 0.001 * linear_grad * linear_grad;
            double next_linear_value = linear_value - linear_lr_t * next_linear_m / (std::sqrt(next_linear_v) + 1.0e-8);
            if (cfg.linear_delta_clip > 0.0) {
                next_linear_value = clamp_value(
                    next_linear_value,
                    linear_base[param_id] - cfg.linear_delta_clip,
                    linear_base[param_id] + cfg.linear_delta_clip
                );
            }
            linear_m[param_id] = static_cast<float>(next_linear_m);
            linear_v[param_id] = static_cast<float>(next_linear_v);
            linear[param_id] = static_cast<float>(next_linear_value);
        }
    }
    ...
    for (int file_id : file_order) {
        ...
        while (cfg.max_records_per_file <= 0 || n_file < static_cast<uint64_t>(cfg.max_records_per_file)) {
            ...
            for (const Record &record : chunk) {
                update_record(
                    record, phase, cfg, linear_base, linear, linear_m, linear_v,
...
PhaseResult train_phase(
    ...
    const std::vector<float> linear_base = linear;
    ...
    best_count_vectors_out.assign(static_cast<size_t>(N_PARAMS_PER_PHASE) * cfg.dim, 0.0f);
    result.best_epoch = 0;

    for (int epoch = 1; epoch <= cfg.epochs; ++epoch) {
        const uint64_t updates = train_one_epoch(
            data_root, phase, cfg.train_ids, epoch, cfg, linear_base, linear, linear_m, linear_v,
            cross_vectors, cross_m, cross_v, cross_initialized,
            same_vectors, same_m, same_v, same_initialized,
            count_vectors, count_m, count_v, count_initialized,
    ...
    for (int param_id = 0; param_id < N_PARAMS_PER_PHASE; ++param_id) {
        const double delta = static_cast<double>(best_linear_out[param_id]) - static_cast<double>(linear_base[param_id]);
        result.linear_max_abs_delta = std::max(result.linear_max_abs_delta, std::abs(delta));
        const int base_q = clamp_value(static_cast<int>(std::llround(static_cast<double>(linear_base[param_id]) * STEP)), -32767, 32767);
        const int best_q = clamp_value(static_cast<int>(std::llround(static_cast<double>(best_linear_out[param_id]) * STEP)), -32767, 32767);
        if (base_q != best_q) {
            ++result.linear_changed_params;
        }
    }
    return result;
}
...
void store_phase_linear_values(
    std::vector<int16_t> &linear_values,
    int phase,
    const std::vector<float> &linear
) {
    const size_t phase_base = static_cast<size_t>(phase) * N_PARAMS_PER_PHASE;
    for (int param_id = 0; param_id < N_PARAMS_PER_PHASE; ++param_id) {
        const int q = static_cast<int>(std::llround(static_cast<double>(linear[param_id]) * STEP));
        linear_values[phase_base + param_id] = static_cast<int16_t>(clamp_value(q, -32767, 32767));
    }
}
...
void write_summary(
    ...
    out << "optimizer=current-linear-trainable joint-tri-type-FM streaming Adam\n";
    ...
    out << "linear_training=initialized_from_input_egev2_and_jointly_updated_with_L2_to_initial_weights\n";
    ...
    out << "linear_lr_scale=" << cfg.linear_lr_scale << "\n";
    out << "linear_delta_clip=" << cfg.linear_delta_clip << "\n";
    ...
    for (const auto &[phase, result] : phase_results) {
        ...
        out << "linear_max_abs_delta=" << result.linear_max_abs_delta << "\n";
        out << "linear_changed_params=" << result.linear_changed_params << "\n";
...
int main(int argc, char **argv) {
    try {
        ...
        std::vector<int16_t> trained_linear_values = linear_values;
        ...
        for (int phase : cfg.phases) {
            ...
            PhaseResult result = train_phase(
                cfg, data_root, phase, linear_values, best_linear,
                best_cross_vectors, best_same_vectors, best_count_vectors
            );
            store_phase_linear_values(trained_linear_values, phase, best_linear);
            ...
            std::cout << "phase " << phase
                      ...
                      << " linear_changed_params " << result.linear_changed_params
                      << " linear_max_abs_delta " << result.linear_max_abs_delta
                      << std::endl;
        }

        write_egev4(cfg, trained_linear_values, phase_cross_vectors, phase_results, cfg.output_cross_egev4, 0);
        write_egev4(cfg, trained_linear_values, phase_same_vectors, phase_results, cfg.output_same_egev4, 1);
        write_egev4(cfg, trained_linear_values, phase_count_vectors, phase_results, cfg.output_count_egev4, 2);

...
既定値は、開始局面あたり512棋譜、16棋譜ごとの反復更新、レベル21、1手あたり2石損まで、1局合計4石損まで、30空きで打ち切りです。これらはコマンドラインオプションで変更できます。
...
Defaults are 512 records per start, iterative updates every 16 records, level 21, per-move loss 2, total loss 4, and cut at 30 empties. These can be changed with command-line options.

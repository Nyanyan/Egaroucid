...
//constexpr int MPC_SUB_DEPTH_VALUE_THRESHOLD = 20;
        ...
        #define GGS_TOURNAMENT_MPC_DEPTH_NUMERATOR 4
    #endif
    #ifndef GGS_TOURNAMENT_MPC_DEPTH_DENOMINATOR
        #define GGS_TOURNAMENT_MPC_DEPTH_DENOMINATOR 11
    #endif
...
inline bool mpc(Search* search, int alpha, int beta, int depth, uint64_t legal, const bool is_end_search, int* v, std::vector<bool*> &searchings) {
    ...
    */
    ...
    if (search_depth == 0) {
        ...
        int static_error = ceil(MPC_ERROR_SCALE * mpct * probcut_sigma(search->n_discs, 0, depth));
#endif

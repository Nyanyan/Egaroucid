...
int16_t pattern_arr_move_ordering[N_PHASES][N_PATTERN_PARAMS_MOVE_ORDERING];
...
inline bool load_eval_file(const char* file, bool show_log){
    ...
    for (int phase_idx = 0; phase_idx < N_PHASES; ++phase_idx){
        ...
        if (fread(pattern_arr[phase_idx] + pattern_starts[0], 2, N_PATTERN_PARAMS_BEFORE_DUMMY, fp) < N_PATTERN_PARAMS_BEFORE_DUMMY){
...
inline bool load_eval_move_ordering_file(const char* file, bool show_log){
    if (show_log)
        std::cerr << "evaluation for move ordering file " << file << std::endl;
    FILE* fp;
    if (!file_open(&fp, file, "rb")){
        std::cerr << "[ERROR] [FATAL] can't open eval " << file << std::endl;
        return false;
    ...
    for (int phase = 0; phase < N_PHASES; ++phase){
        pattern_arr_move_ordering[phase][0] = 0; // memory bound
        if (fread(pattern_arr_move_ordering[phase] + pattern_starts[0], 2, N_PATTERN_PARAMS_MOVE_ORDERING - 1, fp) < N_PATTERN_PARAMS_MOVE_ORDERING - 1){
    ...
    for (int phase = 0; phase < N_PHASES; ++phase){
        for (int i = 1; i < N_PATTERN_PARAMS_MOVE_ORDERING; ++i){
            if (pattern_arr_move_ordering[phase][i] < -SIMD_EVAL_MAX_VALUE_MOVE_ORDERING){
                std::cerr << "[ERROR] evaluation value too low. you can ignore this error. index " << i << " found " << pattern_arr_move_ordering[phase][i] << std::endl;
                pattern_arr_move_ordering[phase][i] = -SIMD_EVAL_MAX_VALUE_MOVE_ORDERING;
            }
            if (pattern_arr_move_ordering[phase][i] > SIMD_EVAL_MAX_VALUE_MOVE_ORDERING){
                std::cerr << "[ERROR] evaluation value too high. you can ignore this error. index " << i << " found " << pattern_arr_move_ordering[phase][i] << std::endl;
                pattern_arr_move_ordering[phase][i] = SIMD_EVAL_MAX_VALUE_MOVE_ORDERING;
            }
            pattern_arr_move_ordering[phase][i] += SIMD_EVAL_MAX_VALUE_MOVE_ORDERING;
        }
...
inline void pre_calculate_eval_constant(){
    ...
    { // eval_move initialization
        ...
        eval_simd_offsets_comp_move_ordering[0] = _mm256_sub_epi32(eval_simd_offsets_comp[0], _mm256_set1_epi32(pattern_starts[8] - pattern_starts[0])); // move ordering uses 8, 9, 10, 11 features
        eval_simd_offsets_comp_move_ordering[1] = _mm256_sub_epi32(eval_simd_offsets_comp[1], _mm256_set1_epi32(pattern_starts[8] - pattern_starts[0])); // move ordering uses 8, 9, 10, 11 features
...
*/
inline bool evaluate_init(const char* file, const char* mo_file, bool show_log){
    bool eval_loaded = load_eval_file(file, show_log);
    if (!eval_loaded){
        std::cerr << "[ERROR] [FATAL] evaluation file not loaded" << std::endl;
        return false;
    }
    bool eval_move_ordering_loaded = load_eval_move_ordering_file(mo_file, show_log);
    if (!eval_move_ordering_loaded){
        std::cerr << "[ERROR] [FATAL] evaluation file for move ordering not loaded" << std::endl;
        return false;
    }
...
/*
    ...
    @return evaluation function conpletely initialized?
*/
bool evaluate_init(const std::string file, std::string mo_file, bool show_log){
    return evaluate_init(file.c_str(), mo_file.c_str(), show_log);
}
...
bool evaluate_init(bool show_log){
    return evaluate_init("resources/eval.egev", "resources/eval_move_ordering.egev", show_log);
}
...
inline __m256i calc_idx8_comp_move_ordering(const __m128i feature, const int i){
    return _mm256_add_epi32(_mm256_cvtepu16_epi32(feature), eval_simd_offsets_comp_move_ordering[i]);
}
...
inline int calc_pattern_move_ordering(const int phase_idx, Eval_features *features){
    const int *start_addr = (int*)pattern_arr_move_ordering[phase_idx];
...
/*
    @brief midgame evaluation function for move ordering
...
inline int mid_evaluate_move_ordering(Search *search){
    ...
    int res = calc_pattern_move_ordering(phase_idx, &search->eval.features[search->eval.feature_idx]);
    res += res >= 0 ? STEP_2 : -STEP_2;
    res /= STEP;
    return res;
}

...
def clip_vector_norm(vec, max_norm):
    ...
    if norm_sq <= max_norm_sq:
        return
    scale = max_norm / math.sqrt(norm_sq)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for rec_idx in order:
            ...
            for param_id in active_ids:
                ...
                clip_vector_norm(vec, args.vector_norm_clip)
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="maximum L2 norm for each FM feature vector; 0 disables norm clipping",
    )
    ...
    if args.vector_norm_clip < 0.0:
        raise ValueError("--vector-norm-clip must be non-negative")
    ...
    with summary.open("w", encoding="utf-8", newline="\n") as f:
        ...
        f.write(f"vector_norm_clip={args.vector_norm_clip}\n")

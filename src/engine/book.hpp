...
class Book{
    private:
        std::mutex mtx;
        std::unordered_map<Board, Book_elem_egbk3, Book_hash> book;
    ...
    public:
        ...
        */
        bool init(std::string file, bool show_log, bool *stop_loading){
            delete_all();
            if (!import_file_bin_egbk3(file, show_log, stop_loading)){ // try egbk3 format
                if (!import_file_bin_egbk2(file, show_log, stop_loading)){ // try egbk2 format
                    return import_file_bin_egbk(file, show_log, stop_loading); // try egbk format
                }
            }
            return true;
        }
        ...
        /*
            ...
            @return book completely imported?
        */
        inline bool import_file_bin_egbk3(std::string file, bool show_log, bool *stop_loading){
            if (show_log)
                std::cerr << "importing " << file << std::endl;
            FILE* fp;
            if (!file_open(&fp, file.c_str(), "rb")){
        ...
        /*
            ...
            @return book completely imported?
        */
        inline bool import_file_bin_egbk2(std::string file, bool show_log, bool *stop_loading){
            if (show_log)
                std::cerr << "importing " << file << std::endl;
            FILE* fp;
            if (!file_open(&fp, file.c_str(), "rb")){

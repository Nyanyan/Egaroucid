/*
    ...
    @file evaluate_experiment_current_fm_dual_type_phase_weight_common.hpp
        Shared loader and scorer for the isolated current-model + phase-weighted dual-type FM experiment
    @date 2026
...
std::array<double, N_PHASES> current_fm_cross_weight_by_phase{};
std::array<double, N_PHASES> current_fm_same_weight_by_phase{};
...
inline std::vector<std::string> current_fm_split_char(const std::string &text, const char delimiter) {
    std::vector<std::string> parts;
    size_t start = 0;
    while (start <= text.size()) {
...
inline bool current_fm_load_egev4(const char *file, bool show_log) {
    ...
    if (parts.size() < 2 || parts[0].empty() || parts[1].empty()) {
        std::cerr << "[ERROR] [FATAL] phase-weighted dual-type current-FM expects -eval cross.egev4@same.egev4[@cross_weight@same_weight[@phase:cross:same,...]]" << std::endl;
        return false;
    }
    try {
        ...
        std::cerr << "[ERROR] [FATAL] invalid default phase-weighted dual-type FM weights: " << e.what() << std::endl;
        return false;
    }
    current_fm_cross_weight_by_phase.fill(current_fm_cross_weight);
    current_fm_same_weight_by_phase.fill(current_fm_same_weight);
    if (parts.size() >= 5 && !parts[4].empty()) {
        const std::vector<std::string> entries = current_fm_split_char(parts[4], ',');
        for (const std::string &entry : entries) {
            const std::vector<std::string> fields = current_fm_split_char(entry, ':');
            if (fields.size() != 3) {
                std::cerr << "[ERROR] [FATAL] invalid phase weight override '" << entry
                          << "'. Expected phase:cross_weight:same_weight" << std::endl;
                return false;
            }
            try {
                const int phase = std::stoi(fields[0]);
                const double cross_weight = std::stod(fields[1]);
                const double same_weight = std::stod(fields[2]);
                if (phase < 0 || N_PHASES <= phase) {
                    std::cerr << "[ERROR] [FATAL] phase weight override phase out of range: " << phase << std::endl;
                    return false;
                }
                current_fm_cross_weight_by_phase[phase] = cross_weight;
                current_fm_same_weight_by_phase[phase] = same_weight;
            } catch (const std::exception &e) {
                std::cerr << "[ERROR] [FATAL] invalid phase weight override '" << entry << "': " << e.what() << std::endl;
                return false;
            }
        }
    }
    if (show_log) {
        std::cerr << "current-model + phase-weighted dual-type FM experiment cross file " << parts[0] << std::endl;
        std::cerr << "current-model + phase-weighted dual-type FM experiment same file " << parts[1] << std::endl;
        std::cerr << "phase-weighted dual-type FM default weights cross=" << current_fm_cross_weight
                  << " same=" << current_fm_same_weight << std::endl;
        if (parts.size() >= 5 && !parts[4].empty()) {
            std::cerr << "phase-weighted dual-type FM overrides " << parts[4] << std::endl;
        }
    }
...
inline int current_fm_score_from_ids(const int phase_idx, const int active_ids[], const int n_active) {
    ...
    const double score = linear
        + current_fm_cross_weight_by_phase[phase_idx] * cross_interaction
        + current_fm_same_weight_by_phase[phase_idx] * same_interaction;

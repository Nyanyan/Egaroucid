...
強制された進行をなぞっている場合、情報エリアに「強制中」と表示されます。
...
バージョン7.8.0から、ヒントの数値につける配色を2つ用意しました。なお、どちらの配色でも最善手を太字で表示しています。
...
終盤20手での各プレイヤーの石損を表示します。バージョン7.8.0で搭載した機能です。
...
生成ボタンを押すと自動で中盤局面が生成されます。生成手数を変更することで、何手目の局面を生成するかを選択できます。また、生成する局面のスコア範囲を設定できるようにしています。下の画像では、18手打った後の局面で黒にとって-38から-21まで、つまり白にとって+21から+38の範囲の局面を生成する設定になっています。ただし、中盤探索で大体スコア範囲に収まるように盤面を生成しているため、厳密に生成盤面がスコア範囲に収まるかは保証できません。

ランダム局面生成はショートカットキーとしてCtrl+Rを割り当てています。
...
この機能では、各着手に対して68%の確率で4石損以下、95%の確率で8石損以下の手を選ぶようにしており、突拍子のない局面は出にくくなっています。
...
なお、ここでは強い手を選ぶ必要がないため、手の評価値はbook登録値または2手読みの評価値を使用しています。

指定された手数までの最後の2手は、指定されたスコア範囲に収まるような手があればその手を、なければスコア範囲に近い手を選ぶようにしています。

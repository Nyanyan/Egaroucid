/*
    ...
    Current-linear fixed pattern-only FM optimizer with streaming chunk
    ...
    interaction and vector updates use only the 64 board-pattern features.
*/
...
bool is_pattern_param(uint32_t param_id) {
    return param_id < static_cast<uint32_t>(N_PATTERN_PARAMS_RAW);
}
...
double predict_record(const std::vector<float> &linear, const std::vector<float> &vectors, const Record &record, int dim, std::vector<double> &sums) {
    std::fill(sums.begin(), sums.end(), 0.0);
    std::vector<double> sums_sq(static_cast<size_t>(dim), 0.0);
    double value = 0.0;
    for (uint32_t param_id : record.active_ids) {
        value += linear[param_id];
        if (!is_pattern_param(param_id)) {
            continue;
        }
...
Metrics compute_metrics(
    ...
    for (int file_id : file_ids) {
        ...
        while (max_records_per_file <= 0 || n_file < static_cast<uint64_t>(max_records_per_file)) {
            ...
            const double pred = predict_record(linear, vectors, record, dim, sums);
...
void update_record(
    ...
    const double pred = predict_record(linear, vectors, record, cfg.dim, sums);
    ...
    for (int file_id : file_order) {
        ...
        while (cfg.max_records_per_file <= 0 || n_file < static_cast<uint64_t>(cfg.max_records_per_file)) {
            ...
            for (const Record &record : chunk) {
                update_record(record, phase, cfg, linear, vectors, m, v, initialized, step, beta1_power, beta2_power, sums);
                ++updates;
            }
            ++chunk_idx;
        }
...
void write_summary(
    ...
    out << "optimizer=current-linear-fixed pattern-only-FM streaming Adam\n";

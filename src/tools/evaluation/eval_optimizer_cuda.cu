...
#define N_THREADS_PER_BLOCK_NEXT_STEP 256
...
int adj_import_train_data(int n_files, char* files[], Adj_Data* host_train_data) {
    int n_data = 0;
    FILE* fp;
...
__global__ void adj_calculate_residual(const float *device_eval_arr, const int n_data, const Adj_Data *device_train_data, float *device_residual_arr, float *device_error_monitor_arr){
...
/*
    @brief GD
*/
__global__ void adj_next_step_gd(const int eval_size, float *device_eval_arr, float *device_residual_arr, float alpha){
    const int eval_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (eval_idx >= eval_size){
        return;
    }
    device_eval_arr[eval_idx] += alpha * 2.0 * device_residual_arr[eval_idx];
}
...
int main(int argc, char* argv[]) {
    ...
    int n_data = adj_import_train_data(argc - 8, test_files, host_train_data);
    ...
    float alpha = beta / n_data;
    uint64_t strt = tim();
    int t = 0;
    while (tim() - strt < msecond){
        adj_reset_residual <<<n_blocks_next_step, N_THREADS_PER_BLOCK_NEXT_STEP>>> (eval_size, device_residual_arr);
        adj_calculate_residual <<<n_blocks_residual, N_THREADS_PER_BLOCK_RESIDUAL>>> (device_eval_arr, n_data, device_train_data, device_residual_arr, device_error_monitor_arr);
        ...
        adj_next_step_gd <<<n_blocks_next_step, N_THREADS_PER_BLOCK_NEXT_STEP>>> (eval_size, device_eval_arr, device_residual_arr, alpha);
        ++t;
    }
    std::cerr << std::endl;

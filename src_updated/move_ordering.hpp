...
short move_ordering_pattern_arr[N_MOVE_ORDERING_PATTERNS][MAX_MOVE_ORDERING_EVALUATE_IDX];
...
inline bool move_ordering_init(){
    ...
    for (pattern_idx = 0; pattern_idx < N_MOVE_ORDERING_PATTERNS; ++pattern_idx){
        if (fread(move_ordering_pattern_arr[pattern_idx], 2, eval_sizes[pattern_idx], fp) < eval_sizes[pattern_idx]){
...
inline int move_evaluate(Board *board, Mobility *mob){
    ...
    return 
        move_ordering_pattern_arr[0][pick_pattern(p, o, mob->flip, 0, 1, 2, 3, 4, 5, 6, 7)] + 
        move_ordering_pattern_arr[0][pick_pattern(p, o, mob->flip, 0, 8, 16, 24, 32, 40, 48, 56)] + 
        move_ordering_pattern_arr[0][pick_pattern(p, o, mob->flip, 7, 15, 23, 31, 39, 47, 55, 63)] + 
        move_ordering_pattern_arr[0][pick_pattern(p, o, mob->flip, 56, 57, 58, 59, 60, 61, 62, 63)] + 

        move_ordering_pattern_arr[1][pick_pattern(p, o, mob->flip, 8, 9, 10, 11, 12, 13, 14, 15)] + 
        move_ordering_pattern_arr[1][pick_pattern(p, o, mob->flip, 1, 9, 17, 25, 33, 41, 49, 57)] + 
        move_ordering_pattern_arr[1][pick_pattern(p, o, mob->flip, 6, 14, 22, 30, 38, 46, 54, 62)] + 
        move_ordering_pattern_arr[1][pick_pattern(p, o, mob->flip, 48, 49, 50, 51, 52, 53, 54, 55)] + 

        move_ordering_pattern_arr[2][pick_pattern(p, o, mob->flip, 16, 17, 18, 19, 20, 21, 22, 23)] + 
        move_ordering_pattern_arr[2][pick_pattern(p, o, mob->flip, 2, 10, 18, 26, 34, 42, 50, 58)] + 
        move_ordering_pattern_arr[2][pick_pattern(p, o, mob->flip, 5, 13, 21, 29, 37, 45, 53, 61)] + 
        move_ordering_pattern_arr[2][pick_pattern(p, o, mob->flip, 40, 41, 42, 43, 44, 45, 46, 47)] + 

        move_ordering_pattern_arr[3][pick_pattern(p, o, mob->flip, 24, 25, 26, 27, 28, 29, 30, 31)] + 
        move_ordering_pattern_arr[3][pick_pattern(p, o, mob->flip, 3, 11, 19, 27, 35, 43, 51, 59)] + 
        move_ordering_pattern_arr[3][pick_pattern(p, o, mob->flip, 4, 12, 20, 28, 36, 44, 52, 60)] + 
        move_ordering_pattern_arr[3][pick_pattern(p, o, mob->flip, 32, 33, 34, 35, 36, 37, 38, 39)] + 
        
        move_ordering_pattern_arr[4][pick_pattern(p, o, mob->flip, 5, 14, 23)] + 
        move_ordering_pattern_arr[4][pick_pattern(p, o, mob->flip, 2, 9, 16)] + 
        move_ordering_pattern_arr[4][pick_pattern(p, o, mob->flip, 40, 49, 58)] + 
        move_ordering_pattern_arr[4][pick_pattern(p, o, mob->flip, 61, 54, 47)] + 

        move_ordering_pattern_arr[5][pick_pattern(p, o, mob->flip, 4, 13, 22, 31)] + 
        move_ordering_pattern_arr[5][pick_pattern(p, o, mob->flip, 3, 10, 17, 24)] + 
        move_ordering_pattern_arr[5][pick_pattern(p, o, mob->flip, 32, 41, 50, 59)] + 
        move_ordering_pattern_arr[5][pick_pattern(p, o, mob->flip, 60, 53, 46, 39)] + 

        move_ordering_pattern_arr[6][pick_pattern(p, o, mob->flip, 3, 12, 21, 30, 39)] + 
        move_ordering_pattern_arr[6][pick_pattern(p, o, mob->flip, 4, 11, 18, 25, 32)] + 
        move_ordering_pattern_arr[6][pick_pattern(p, o, mob->flip, 24, 33, 42, 51, 60)] + 
        move_ordering_pattern_arr[6][pick_pattern(p, o, mob->flip, 59, 52, 45, 38, 31)] + 

        move_ordering_pattern_arr[7][pick_pattern(p, o, mob->flip, 2, 11, 20, 29, 38, 47)] + 
        move_ordering_pattern_arr[7][pick_pattern(p, o, mob->flip, 5, 12, 19, 26, 33, 40)] + 
        move_ordering_pattern_arr[7][pick_pattern(p, o, mob->flip, 16, 25, 34, 43, 52, 61)] + 
        move_ordering_pattern_arr[7][pick_pattern(p, o, mob->flip, 58, 51, 44, 37, 30, 23)] + 

        move_ordering_pattern_arr[8][pick_pattern(p, o, mob->flip, 1, 10, 19, 28, 37, 46, 55)] + 
        move_ordering_pattern_arr[8][pick_pattern(p, o, mob->flip, 6, 13, 20, 27, 34, 41, 48)] + 
        move_ordering_pattern_arr[8][pick_pattern(p, o, mob->flip, 8, 17, 26, 35, 44, 53, 62)] + 
        move_ordering_pattern_arr[8][pick_pattern(p, o, mob->flip, 57, 50, 43, 36, 29, 22, 15)] + 

        move_ordering_pattern_arr[9][pick_pattern(p, o, mob->flip, 0, 9, 18, 27, 36, 45, 54, 63)] + 
        move_ordering_pattern_arr[9][pick_pattern(p, o, mob->flip, 7, 14, 21, 28, 35, 42, 49, 56)];
}
...
inline void move_ordering(Search *search, vector<Mobility> &move_list){
    ...
    for (Mobility &mob: move_list){
        ...
        */
            mob.value = move_evaluate(&search->board, &mob);
    }

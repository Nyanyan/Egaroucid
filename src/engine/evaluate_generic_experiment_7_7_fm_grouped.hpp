/*
    Egaroucid Project

    @file evaluate_generic_experiment_7_7_fm_grouped.hpp
        Grouped-vector 7.7 beta + FM evaluation experiment without AVX2
    @date 2026
    @author Takuto Yamana, Codex
    @license GPL-3.0-or-later
*/

#pragma once
#ifndef EVALUATE_EXPERIMENT_7_7_FM_FAST_COMMON_HEADER
    #define EVALUATE_EXPERIMENT_7_7_FM_FAST_COMMON_HEADER "evaluate_experiment_7_7_fm_grouped_common.hpp"
#endif
#include "evaluate_generic_experiment_7_7_fm_fast.hpp"

/*
    ...
    @file current_fm_tri_type_count_by_type_stream12_score_speed_check.cpp
        Speed check for current-model count-by-pattern-type tri-type FM scoring
        with the streaming dim12 path.
    @date 2026
...
#include "../../../engine/evaluate_experiment_current_fm_tri_type_count_by_type_stream12_common.hpp"
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::cerr << "usage: current_fm_tri_type_count_by_type_stream12_score_speed_check <cross.egev4@same.egev4@count.egct@cross_weight@same_weight@count_weight> [iterations] [case_count]" << std::endl;
        return 1;
    }
    ...
    run_benchmark("count_by_type_fm_scalar", cases, iterations, current_fm_score_from_ids_quant);
    run_benchmark("count_by_type_fm_stream12_checked", cases, iterations, current_fm_score_from_ids_stream12);
#if USE_SIMD
    current_fm_stream12_prepare_sparse_masks();
    run_benchmark("count_by_type_fm_stream12_sparse", cases, iterations, [](const int phase, const int ids[], const int) {

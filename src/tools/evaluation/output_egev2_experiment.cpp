...
int main(int argc, char* argv[]){
    if (argc < 2){
        std::cerr << "input [n_phases] [eval_max=" << EVAL_MAX << "] [model_dir=trained] [out_file=model_dir/eval.egev2] [n_params_if_missing]" << std::endl;
        return 1;
    }
    ...
    std::string out_file = join_path(model_dir, "eval.egev2");
    if (argc >= 5){
        out_file = argv[4];
    }
    int n_params_if_missing = -1;
    if (argc >= 6){
        n_params_if_missing = atoi(argv[5]);
    }
    std::cerr << "n_phases " << n_phases << " eval_max " << eval_max << " model_dir " << model_dir << " out_file " << out_file << std::endl;
    std::ofstream fout;
    ...
    for (int phase = 0; phase < n_phases; ++phase){
        ...
        if (ifs.fail()){
            ...
            if (n_params == -1){
                if (n_params_if_missing > 0){
                    n_params = n_params_if_missing;
                    std::cerr << "use n_params_if_missing " << n_params << std::endl;
                } else {

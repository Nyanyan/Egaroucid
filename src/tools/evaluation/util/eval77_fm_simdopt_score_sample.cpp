/*
    ...
    @file eval77_fm_simdopt_score_sample.cpp
        Score sampler for SIMD/Generic comparison of the 7.7 beta + FM SIMDOPT evaluator.
    @date 2026
...
#if !defined(EVALUATE_EXPERIMENT_7_7_FM_SIMDOPT) && !defined(EVALUATE_EXPERIMENT_7_7_FM_SIMDOPT_MMAP) && !defined(EVALUATE_EXPERIMENT_7_7_FM_SUBSET_SIMDOPT) && !defined(EVALUATE_EXPERIMENT_7_7_FM_FAST) && !defined(EVALUATE_EXPERIMENT_7_7_FM_FAST_SUBSET)
...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::cerr << "usage: eval77_fm_simdopt_score_sample <eval.egev4> [positions] [seed]" << std::endl;
        return 1;
    }
    ...
    flip_init();
    last_flip_init();
    if (!evaluate_init(eval_file, "bin/resources/eval_move_ordering_end.egev", false)) {
        return 1;
    }

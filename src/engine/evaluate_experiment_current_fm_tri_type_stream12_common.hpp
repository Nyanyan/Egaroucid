...
std::array<std::vector<uint8_t>, N_PHASES> current_fm_stream12_count_nonzero_mask;
...
inline void current_fm_stream12_make_nonzero_mask(
    ...
    const std::vector<int8_t> &vectors = src.vector_quant_by_phase[phase];
    if (vectors.empty()) {
        dst.clear();
        return;
    }
    dst.assign(CURRENT_FM_N_PARAMS_PER_PHASE, 0);
    for (int param = 0; param < CURRENT_FM_N_PARAMS_PER_PHASE; ++param) {
        const size_t base = (size_t)param * src.dim;
        uint8_t nonzero = 0;
...
inline bool current_fm_stream12_prepare_sparse_masks() {
    ...
    for (int phase = 0; phase < N_PHASES; ++phase) {
        current_fm_stream12_make_nonzero_mask(current_fm_cross_file, phase, current_fm_stream12_cross_nonzero_mask[phase]);
        current_fm_stream12_make_nonzero_mask(current_fm_same_file, phase, current_fm_stream12_same_nonzero_mask[phase]);
        current_fm_stream12_make_nonzero_mask(current_fm_count_file, phase, current_fm_stream12_count_nonzero_mask[phase]);
    }
...
inline int current_fm_score_from_ids_stream12_sparse_unchecked(const int phase_idx, const int active_ids[]) {
    ...
    const uint8_t *count_mask = current_fm_stream12_count_nonzero_mask[phase_idx].empty()
        ? nullptr
        : current_fm_stream12_count_nonzero_mask[phase_idx].data();
    ...
    for (int i = 0; i < CURRENT_FM_N_PATTERN_FEATURES; ++i) {
        ...
        if (count_mask != nullptr && count_mask[id]) {
            __m256i count_lo;
            __m256i count_hi;
            current_fm_load_dim12_ptr(count_ptr, id, count_lo, count_hi);
            count_pattern_sum_lo = _mm256_add_epi32(count_pattern_sum_lo, count_lo);
    ...
    const int count_id = active_ids[CURRENT_FM_N_PATTERN_FEATURES];
    linear_quant_sum += linear_ptr[count_id];
    if (count_mask != nullptr && count_mask[count_id]) {
        current_fm_load_dim12_ptr(count_ptr, count_id, count_vec_lo, count_vec_hi);
    }

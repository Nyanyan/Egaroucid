...
*/
Parallel_task ybwc_do_task_nws(uint64_t player, uint64_t opponent, int_fast8_t n_discs, uint_fast8_t parity, uint_fast8_t mpc_level, bool is_presearch, thread_id_t thread_id, int parent_alpha, const int depth, uint64_t legal, const bool is_end_search, const bool use_full_evaluation, uint_fast8_t policy, int move_idx, std::vector<bool*> searchings, bool *n_searching) {
    ...
    search.eval77_fm_use_full_evaluation = use_full_evaluation;
#else
...
inline int ybwc_split_nws(Search *search, int parent_alpha, const int depth, uint64_t legal, const bool is_end_search, std::vector<bool*> &searchings, bool *n_searching, uint_fast8_t policy, const int n_remaining_moves, const int move_idx, const int running_count, std::vector<std::future<Parallel_task>> &parallel_tasks) {
    ...
    if (
        ...
        if (is_searching(searchings)) {
            bool pushed;
            parallel_tasks.emplace_back(thread_pool.push(search->thread_id, &pushed, std::bind(&ybwc_do_task_nws, search->board.player, search->board.opponent, search->n_discs, search->parity, search->mpc_level, search->is_presearch, search->thread_id, parent_alpha, depth, legal, is_end_search,
#if defined(EVALUATE_EXPERIMENT_7_7_FM_SUBSET_MIDGAME_SEARCH_ONLY)
                search->eval77_fm_use_full_evaluation,
#else
                false,
#endif
                policy, move_idx, searchings, n_searching)));
            if (pushed) {
                #if USE_YBWC_SPLIT_STATISTICS

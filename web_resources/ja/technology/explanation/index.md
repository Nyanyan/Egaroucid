...
評価関数は精度と速さのトレードオフと書きましたが、ここでは精度について注目します。評価関数の複雑さは精度にどれほど貢献するのでしょうか？どれくらいの複雑さであればどれくらいの精度が期待できるでしょうか？ということを考えます。

ここでは、モデルの複雑さを単にパラメータ数と言い換えて考えます。厳密には、オセロというゲームにおいて絶対にありえない局面だけ(例えば真ん中の4マスが空いた局面など)に対応するパラメータも存在するのであまり丁寧な議論ではありませんが、そこには目をつぶります。

このトピックを考える前に、オセロにおいて登場し得る局面の数(状態空間の大きさ)を推定する必要があります。これは[状態空間の大きさの推定](./#ゲームの特性_状態空間の大きさの推定)に詳しく書きました。
...
と考えることができます。第一項は最初に埋まっている4マスの組み合わせ、第二項は残りの60マスで$p$マスを石で埋める埋め方、第三項は$p$手目までで埋まった石の色の組み合わせです。これを$0\leq p \leq 60$で計算すると、以下のグラフのようになります。序盤から40手目付近までで局面の数が増えて、それ以降は終局に近づくにつれて局面数が少し減っています。青線は参考として書いた上界です。また、$p \leq 12$までは上で序盤の展開数を全て求めたので厳密な値がわかります。それもプロットしてみました。この上界はあまり精度が良くないとわかります。

<div class="centering_box">
    <img class="pic2" src="img/state_space_complexity_0.png">
</div>

さて、ここから初期盤面からの到達可能性を考慮して、上限の推定精度を上げましょう。そのために、盤面上に$n$個の石が存在するときに合法手になり得る空きマス数$e(n)$を考えます。これを使って最終的に${}_{60}\mathrm{C}_p$の部分を置き換えます。

まず、$n$個の石があるときの空きマス数が$64-n$個なので、$e(n) \leq 64-n$​です。そして、オセロで合法手になりうる空きマスに石を追加して、8近傍で接している空きマス数を最大化する局面を計算機によって求めます。ここで「合法手になりうる空きマス」は、既存の石に8近傍で接していて、かつその方向に石が2枚以上ある空きマスとします。これは、石を挟むには直線上に2つ以上の石が連続していなければならないからです。また、序盤12手(石数は16個)はあり得る展開が全列挙できるので、序盤12手だけ展開を全列挙して、そこから石を連結していくようにしました。計算時間との兼ね合いで$n \leq 20$の場合についてのみ計算を行いました。なお、対局開始から12手、つまり$n \leq 16$では$e(n)$は合法手数の厳密な最大値を計算しました。



見積もった$e(n)$を使って、$p$手目における合法な局面数$s(p)$を考え直してみると、

$s(p)=2^4 \times \prod_{n=4}^{4+p}e(n) \times 2^p$

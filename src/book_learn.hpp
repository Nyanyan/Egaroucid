...
int book_learn_search(Board board, int level, const int book_depth, int alpha, int beta, int error_sum, const int expected_error, const int adopt_error_sum, Board *board_copy, uint64_t *strt_tim, string book_file, string book_bak){
    if (!global_searching)
    ...
    for (uint_fast8_t cell = first_bit(&legal); legal && alpha <= beta; cell = next_bit(&legal)){
        calc_flip(&flip, &board, cell);
        board.move(&flip);
            board.copy(board_copy);
            g = book.get(&board);
            if (g < -SCORE_MAX || SCORE_MAX < g){
                g = -book_learn_calc_value(board, level, -alpha, -alpha + expected_error);
                if (-SCORE_MAX <= g && g <= SCORE_MAX && g >= alpha - expected_error){
                    g = -book_learn_search(board, level, book_depth, -beta, -alpha, error_sum + min(0, alpha - g), expected_error, adopt_error_sum, board_copy, strt_tim, book_file, book_bak);
                    if (-SCORE_MAX <= g && g <= SCORE_MAX){
                        if (alpha <= g){
                            alpha = g;
                            alpha_updated = true;
    ...
    if (alpha_updated && alpha <= beta && global_searching){

...
inline int calc_mobility_pattern(const int phase_idx, const uint64_t player_mobility, const uint64_t opponent_mobility){
    uint64_t pp = player_mobility & 0x0703010000010307ULL;
    uint64_t oo = opponent_mobility & 0x0703010000010307ULL;
    ...
    o2 = oo >> 11;
    int res = 
        eval_mobility_pattern[phase_idx][0][p1][o1] + 
        eval_mobility_pattern[phase_idx][0][p2][o2];
    pp = horizontal_mirror(player_mobility) & 0x0703010000010307ULL;
    oo = horizontal_mirror(opponent_mobility) & 0x0703010000010307ULL;
    pp *= 0x0000000000200841ULL;
    oo *= 0x0000000000200841ULL;
    pp &= 0x3F0000000001F800ULL;
    oo &= 0x3F0000000001F800ULL;

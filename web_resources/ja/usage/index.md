...
最終更新: 2025/08/16 Egaroucid 7.7.0
...
生成された局面がどれくらい形勢の傾いたものになるかを「スコア範囲」で設定できます。上の画像だと±5となっていますが、これは生成された局面が黒+5から白+5までに収まるようにするという意味です。

スコア範囲は生成された局面のみについての設定なので、その局面に至る途中の局面ではスコア範囲を超える場合もあります。

また、内部的にはレベル15を使ってスコア範囲に盤面が収まるかを判定しているため、厳密に生成盤面がスコア範囲に収まるかは保証できません。
...
バージョン7.7.0からはフォルダ分けに対応しました。フォルダ間の移動はフォルダのダブルクリックで行えます。フォルダ作成機能は<a href="#入出力_出力 > 対局保存">対局保存</a>機能の中にあります。また、フォルダや対局をドラッグ&ドロップで任意のフォルダに移動できます。

...
レベルが2種類混在しているのは、Book制作上の都合です。

Egaroucidのレベル27は28手完全読み、つまり深さ32のところでレベル27を使って計算すると、その値は狂いのない完全読みした評価値となります。この性質を使って、深さ31まではレベル17で高速に計算し、深さ32のところでレベル27を使って完全読みさせています。これらの結果を使ってBook修正を行い、高速な学習と高精度なBookを両立させました。

手元で学習したBookを、主要な定石の最善進行、最善分岐あたりを残して小さくまとめてから公開したものです。
...
深さ30はBook末端で完全読みさせても良いですが、それなりに計算時間がかかるので99%や98%の読み切りを使うのでも良いかと思います。それぞれサンプル設定を紹介します。
...
28手完全読みが簡単にできるくらいのPCであれば、以下2種類くらいが良いと思います。作者はCore i9 13900Kでレベル17と32を使って学習させています。この設定だと、うまくやればBook精度の評価がAのBookが作れます。
...
28手を完全読みさせず、99%や98%の読み切りを使うので良ければ、以下の設定でも良いと思います。

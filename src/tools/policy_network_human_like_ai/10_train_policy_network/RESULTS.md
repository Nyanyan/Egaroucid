...
- Current completed total: 13,845 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 13,845 position
samples:
...
- 現在の完了数: 13,845 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭13,845局面サンプルの top-1 symmetric accuracy:

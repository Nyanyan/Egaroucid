...
- Current completed total: 15,425 / 8,035,282 position samples.
...
Current `partial_merged` top-1 symmetric accuracy on the first 15,425 position
samples:
...
- 現在の完了数: 15,425 / 8,035,282 局面サンプル。
...
現在の `partial_merged` における先頭15,425局面サンプルの top-1 symmetric accuracy:

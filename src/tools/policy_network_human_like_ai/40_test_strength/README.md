...
全組合せには同じset indexで同じXOT局面を割り当てる。そのため、ある参加者の15相手分を単純に7,500個の独立標本としてまとめることはできない。参加者全体のスコアは記述値とする。Eloは0%・100%で任意の無限大や固定clip値にならないよう、対戦済みの各組合せへ0.5勝・0.5敗を加えた有限な相対値である。未対戦の組合せは推定の重みを0とし、対戦済み組合せのグラフが全参加者を連結するまでは途中Eloを表示しない。

最終Eloの95%信頼区間は、完了後に`strength_games.jsonl`を再解析して求める。set index、すなわちXOT局面をクラスタとし、同じ復元抽出列を全組合せへ適用するBCaクラスターブートストラップを使う。これにより、同じXOTを共有した組間の相関を保つ。2色の実対局や各組合せを別々に再標本化してはならない。
...
対戦が完了したらElo 95%信頼区間を生成する。既定では20,000反復、PCG64、seed 613を使う。元の棋譜、manifest、既存集計は変更せず、補助CSVとJSONだけを出力する。

```powershell
python src/tools/policy_network_human_like_ai/40_test_strength/reanalyze_strength_elo.py `
  --output-dir src/tools/policy_network_human_like_ai/40_test_strength/output/xot_500sets_16players `
...
この表を参加者全体の7,500セットへそのまま適用してはならない。15組合せは同じ500局面列を共有するため相関がある。最終Eloには、set index/XOT局面をクラスタとして全組合せを同時に20,000回再標本化し、各反復で16参加者平均を1500へ中心化したBCa 95%信頼区間を別途出力する。

この区間は参加者ごとのpointwise区間であり、16参加者すべてを同時に95%で覆う区間ではない。対象は同じXOT母集団から別の局面列を抽出した場合の変動であり、モデル再学習、探索条件変更、別のopening corpusの不確実性は含まない。2参加者の差は、個別区間の重なりではなく、保存された同一bootstrap反復内のElo差で評価する。
...
| `strength_summary.csv` | 参加者別の記述的セット成績、実対局成績、Elo点推定 |
...
| `strength_elo_ci95.csv` | Elo点推定とXOTクラスタBCa 95%信頼区間 |
| `strength_elo_ci95.json` | Elo区間の手法、seed、補正値、由来、結果 |
| `strength_elo_bootstrap_replicates.csv` | 各bootstrap反復の全参加者Elo |
| `strength_elo_bootstrap_cluster_indices.csv` | 各反復で抽出したset index列 |
...
`RESULTS.md`の先頭には2026-07-21に現行実装・16参加者・50セットで完走した本番結果を記録している。後半の2026-07-18節は旧実装・17参加者の過去結果であり、現行結果とは別の実験である。

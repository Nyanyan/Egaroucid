...
def reduce_traindata_nums(nums):
    ...
    for num in nums:
        ...
        mx = board_n_moves[str(num)][1]
        phase_int = int(phase)
        if (mn + 6 <= phase_int <= mx) or (num in use_all_depth_data) or (phase_int <= 12 + 6): # ランダム打ちからN手以上打ったデータのみ使う (phase 12+N以前は確保できるデータ数が少ないので全部使う)
            res.append(num)
    return res
...
executable = 'eval_optimizer_cuda_12_2_0_20241125_1_7_5_20260513_all_off.exe'
